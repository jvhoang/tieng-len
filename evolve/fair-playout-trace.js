'use strict';
/**
 * Trace one fair dual game: freeze path + chall decision each live seat step.
 *   FREEZE=v91 CHALL=p_w14_bropair SEED=20270684 SEAT=0 node evolve/fair-playout-trace.js
 */
...
function seatOpts() {
  return {
    ...
    exactExploit: false, exploit: true, softSamples: soft, maxBranch: 12, mode: 'auto', strongSelf: false
  };
}
...
function cardLabel(c) {
  return ('3456789TJQKA2'[c.rank] || '?') + ('SHDC'[c.suit] || '?');
}
...
while (!state.roundOver && steps < 240) {
  ...
  if (cp === liveSeat) {
    ...
    row.curTop = state.currentCombo && state.currentCombo.top ? state.currentCombo.top.rank : null;
    row.freeze = playLabel(fz);
    row.chall = playLabel(ch);
    row.diff = playSig(fz) !== playSig(ch);
    row.hand = state.players[cp].hand.map(cardLabel).join(' ');
    mv = fz; // follow freeze path for shared history of still-loss diagnosis
  } else {
    mv = withDetRng(key, function () { return freeze.getAIMove(state, cp, opts); });
    row.freeze = playLabel(mv);
    row.chall = '(opp freeze)';
  }
  rows.push(row);
...
const out = {
  seed: seed, liveSeat: liveSeat, chall: challTag, freeze: freezeTag,
  winner: winner, liveWin: winner === liveSeat, steps: steps,
  diffs: rows.filter(function (r) { return r.diff; }),
  rows: rows
};
