...
function heuristicOrder(legals, state, myIdx) {
  return legals.slice().sort((a, b) => {
    ...
    const ta = ca ? ca.top.rank : 99; const tb = cb ? cb.top.rank : 99;
    return ta - tb;
  });
}
...
class MCTSNode {
  constructor(state, playerIdx, move = null, parent = null) {
    this.state = state; this.player = playerIdx; this.move = move; this.parent = parent;
    this.children = []; this.visits = 0; this.wins = 0; this.untried = null;
  }
}

function getWinnerFromFinish(finishOrder, numPlayers) { return finishOrder && finishOrder.length ? finishOrder[0] : null; }
...
function rolloutPolicy(state, myIdx) {
  ...
  while (!s.roundOver && steps < 120) {
    ...
    if (leg.length === 0) { s = pass(s, cp); } else { const choice = getExpertMove(s, cp) || leg[0]; s = applyPlay(s, cp, choice); }
...
function runMCTS(rootState, myIdx, iterations = 1200) {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    while (node.untried == null && node.children.length > 0 && !s.roundOver) {
      let best = null; let bestU = -Infinity; const C = 1.4;
      ...
      node = best;
      if (node.move) { const advPlayer = node.player; s = applyPlay(s, advPlayer, node.move); }
    }
    if (!s.roundOver) {
      ...
      if (node.untried && node.untried.length) {
        ...
        node.children.push(child); node = child; s = newS;
      }
    }
    const outcome = rolloutPolicy(s, myIdx);
    let cur = node;
    while (cur) { cur.visits++; cur.wins += outcome; cur = cur.parent; }
  }
  let bestChild = null; let bestV = -1;
  for (const ch of root.children) { const v = ch.visits ? (ch.wins / ch.visits) : 0; if (v > bestV || bestChild === null) { bestV = v; bestChild = ch; } }
...
function getAIMove(state, myIdx, opts = {}) {
  ...
  let iters = difficulty === 'easy' ? 60 : (difficulty === 'medium' ? 180 : 600);
  const isFast = (typeof process !== 'undefined' && process.env && (process.env.TIENLEN_TEST_FAST || process.env.NODE_ENV==='test'));
  if (isFast || (opts.iterations && opts.iterations < 200)) { iters = Math.min(iters, opts.iterations || 40); }
  try { const mv = runMCTS(state, myIdx, iters); if (mv) return mv; } catch (e) {}
  return getExpertMove(state, myIdx);
}

function aiChoosesNonTrivial(state, myIdx) { const mv = getAIMove(state, myIdx, {difficulty: 'hard'}); return mv && mv.length > 0; }

const TienLenAI = { getAIMove, getExpertMove, runMCTS, aiChoosesNonTrivial, scorePosition, getLearnedWeights: () => learnedWeights.slice(), selfPlayLearn };
...
function selfPlayLearn(numGames = 8) {
  ...
  for (let g=0; g<numGames; g++) {
    let s = engine.createGameState(4, 900 + g); let steps=0;
      ...
      if (leg.length === 0) { s = engine.pass(s, cp); } else {
        if (leg.length > 1) {
          choice = leg.reduce((best, pl) => {
            ...
            const sc = learnedScore(ns, cp); const bsc = best ? learnedScore(best.ns, cp) : -Infinity;
