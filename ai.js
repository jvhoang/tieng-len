/**
 * Tiến Lên AI - Hybrid Expert + MCTS + lightweight self-play ML (wired)
 */
...
function scorePosition(state, myIdx) {
  ...
  try {
    if (typeof learnedScore === 'function') score = score * 0.6 + learnedScore(state, myIdx) * 0.4;
  } catch(_) {}
  return score;
}
...
function learnedScore(state, myIdx) {
  ...
  let s = 0; for (let i=0; i<f.length; i++) s += f[i] * learnedWeights[i]; return s;
}
function selfPlayLearn(numGames=5) {
  for (let g=0; g<numGames; g++) {
    let s = engine.createGameState(4, 900+g);
    let steps=0;
    while (!s.roundOver && steps < 60) {
      const cp = s.currentPlayer;
      ...
      s = engine.applyPlay(s, cp, choice); s.isFirstLead=false; steps++;
    }
