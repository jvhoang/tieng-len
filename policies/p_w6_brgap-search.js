...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    // W6D BR_NEAR_OVK: combat single near-rate pool — min-top if gap>=2
    if (cur && cur.type === 'single' && details.length) {
      ...
      var sings = pool.filter(function (d) { var a = actBySig[d.sig]; return a && a.length === 1; });
      if (sings.length >= 2) {
        var minT = 99, maxT = -1;
        for (var si = 0; si < sings.length; si++) {
          var t = top(actBySig[sings[si].sig]);
          if (t < minT) minT = t; if (t > maxT) maxT = t;
        }
        if (maxT - minT >= 2) {
          sings.sort(function (da, db) {
          ...
          if (actBySig[sings[0].sig] !== undefined) bestPlay = actBySig[sings[0].sig];
        }
      }
    }
    return {
