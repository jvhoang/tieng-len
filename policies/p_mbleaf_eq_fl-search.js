...
(function (root, factory) {
  ...
  function shallowSelfPick(state, myIdx) {
    ...
    // LEAF_EQ_FORCEWIN: among forced wins, pick max residual pairs → min top → min sbc
    var wins = [];
    for (i = 0; i < leg.length; i++) {
      var n = applyPlayFast(state, myIdx, leg[i]);
      n.isFirstLead = false;
      if (exploitPlayoutLeaf(n, myIdx, 200) >= 0.99) wins.push(leg[i]);
    }
    if (wins.length) {
      function top(p){var t=-1;for(var j=0;j<p.length;j++)if(p[j].rank>t)t=p[j].rank;return t;}
