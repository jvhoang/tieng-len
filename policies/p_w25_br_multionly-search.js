...
(function (root, factory) {
  ...
  function expertScore(play, state, myIdx) {
    ...
    if (!cur) {
      ...
      if (cur.type === 'single' && com.type === 'single') {
        ...
        // W24 p_w24_ex_climbtax: holdout recon — tax high climbs over low faces
        // when residual 2s/control remain (JH→QS→KS burn class on freeze-identical)
        if (!usesTwo && gap >= 2 && com.top.rank >= 10 && curTop <= 6 && handLen >= 6) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    // W25 p_w25_br_multionly: holdout recon — when facing multi, BR only scores
    // same-type multi answers (force contest; no pass / cross-type noise).
    // Stacks on climbtax. Not W16 pass (opposite: force play multi).
    var forceMultiOnly = false;
    if (
      cur &&
      (cur.type === 'pair' || cur.type === 'triple' || cur.type === 'seq' || cur.type === 'doubleseq') &&
      !playIsBomb(cur.cards || []) &&
      hand.length >= 5 &&
      hand.length <= 12
    ) {
      var multiOnly = [], iM, pM, cM;
      for (iM = 0; iM < leg.length; iM++) {
        pM = leg[iM];
        cM = detectCombo(pM);
        if (cM && cM.type === cur.type && !playHasTwo(pM) && !playIsBomb(pM)) multiOnly.push(pM);
      }
      if (multiOnly.length >= 1) {
        leg = orderLegals(multiOnly, state, myIdx);
        forceMultiOnly = true;
      }
    }
    ...
    if (forceMultiOnly) allowPass = false; // never pass when same-type multi exists
