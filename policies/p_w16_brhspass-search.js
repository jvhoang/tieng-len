...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    // W16 p_w16_brhspass: gold 0501/0510/0547 — multi only high-smash cheap → pass unique
    var hiSmashPass = false;
    if (cur && cur.type !== 'single' && !playIsBomb(cur.cards || []) &&
        hand.length >= 9 && oppMinHand(state, myIdx) >= 5 &&
        (cur.top ? cur.top.rank : 0) <= 9) {
      var src = leg.slice();
      if (src.length) {
        var infoP = analyzeHand(hand);
        var canRetake = (infoP.twos >= 1 || infoP.control >= 2);
        // mid pair-backs also count as re-take potential
        var midPairs = 0, rP;
        var byRP = {};
        for (var hi = 0; hi < hand.length; hi++) byRP[hand[hi].rank] = (byRP[hand[hi].rank] || 0) + 1;
        for (rP = 6; rP <= 11; rP++) if ((byRP[rP] || 0) >= 2) midPairs++;
        if (midPairs >= 2) canRetake = true;
        if (canRetake) {
          var allHi = true, iH, pH, sbcH;
          for (iH = 0; iH < src.length; iH++) {
            pH = src[iH];
            if (!pH) continue;
            sbcH = structureBreakCost(hand, pH);
            var usesHiPair = false;
            if (pH.length >= 2) {
              var tr = topRank(pH);
              if (tr >= 9) usesHiPair = true;
            }
            if (sbcH < 8 && !usesHiPair && !playHasTwo(pH)) { allHi = false; break; }
          }
          if (allHi) { hiSmashPass = true; leg = []; }
        }
      }
    }
    if (leg.length > maxBranch) leg = leg.slice(0, maxBranch);

    var allowPass = hiSmashPass || (!!cur && cheapLegals(
    ...
    if (!actions.length) actions.push(null);
