...
(function (root, factory) {
  ...
  /** W72 residualAfter: pairR / maxRun / sbc after removing play cards. */
  function residualAfter(hand, play) {
    var used = Object.create(null), i, r, byR = Object.create(null);
    if (!play || !play.length) return { pairR: 0, maxRun: 0, sbc: 0 };
  ...
  /** W72 mulres_uniq: combat residual-max multi when unique residual winner
   *  disagrees with orderLegals min-score. Expert∧BR dual-mirror.
   *  SoftN FORBIDDEN. After banked combat micros; combat-only (no FREE). */
  function pickMulResUniq(hand, cur, leg, state, cp) {
    if (!cur || cur.type === 'single') return null;
    if (playIsBomb(cur.cards || [])) return null;
    var curTop = cur.top ? cur.top.rank : 0;
    if (curTop >= 11) return null;
    var handLen = hand.length, omin = oppMinHand(state, cp);
    if (handLen < 7 || omin < 4) return null;
    if (cur.type !== 'pair' && cur.type !== 'triple' &&
        cur.type !== 'seq' && cur.type !== 'doubleseq') return null;

    var pool = [], i, p, c;
    for (i = 0; i < leg.length; i++) {
      p = leg[i];
      if (!p || playHasTwo(p) || playIsBomb(p) || playIsExpensive(p)) continue;
      c = detectCombo(p);
      if (c && c.type === cur.type) pool.push(p);
    }
    ...
    var w = ranked[0], r2 = ranked[1];
    var rw = residualAfter(hand, w), rr = residualAfter(hand, r2);
    // uniqueness: strict residual structure win
    if (rw.pairR === rr.pairR && rw.maxRun === rr.maxRun) return null;
    var freezePick = orderLegals(pool, state, cp)[0];
    if (playSig(w) === playSig(freezePick)) return null; // must-disagree
    return w;
  }
  ...
  function expertPolicy(state, cp) {
    ...
    // W72 mulres_uniq: residual-max unique multi after pairhi (before mulowg min)
    var mru = pickMulResUniq(hand, cur, leg, state, cp);
    if (mru) return { play: mru };
    ...
    if (cheap.length) {
      ...
      var mruC = pickMulResUniq(hand, cur, cheap, state, cp);
      if (mruC) return { play: mruC };
      if (
        cur &&
        cur.type !== 'single' &&
        !playIsBomb(cur.cards || []) &&
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      var mruBR = (!sbc0BR && !meBR && !euBR && !ajuBR && !qpcBR && !shBR && !shrBR && !shr13BR && !saBR && !s5BR && !smuBR && !phBR) ? pickMulResUniq(hand, cur, leg, state, myIdx) : null;
      if (sbc0BR) {
        leg = [sbc0BR];
      } else if (meBR) {
        leg = [meBR];
