...
(function (root, factory) {
  ...
  function expertPolicy(state, cp) {
    ...
    // W8D FORBID_OVERKILL: combat singles — drop gap>2 when gap<=2 exists
    if (cheap.length && cur.type === 'single' && cur.top) {
      var cTop = cur.top.rank;
      var sings = cheap.filter(function (p) { return p && p.length === 1; });
      if (sings.length >= 2) {
        var minG = 99, i;
        for (i = 0; i < sings.length; i++) {
          var g = sings[i][0].rank - cTop;
          if (g >= 1 && g < minG) minG = g;
        }
        if (minG <= 2) {
          sings = sings.filter(function (p) { return (p[0].rank - cTop) <= 2; });
          sings.sort(function (a, b) {
            var ga = a[0].rank - cTop, gb = b[0].rank - cTop;
