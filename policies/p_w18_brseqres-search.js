...
(function (root, factory) {
  ...
  // W18 residual structure score after removing play (pairR*10 + maxRun)
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      if (ch.length) leg = ch;
      // W18 p_w18_brseqres: gold 0503/0519 — force max residual same-len seq
      if (cur && cur.type === 'seq') {
        var L = cur.cards.length, best = -1, keep = [], nSeq = 0, iS, pS, dS, scS;
        for (iS = 0; iS < leg.length; iS++) {
          pS = leg[iS]; if (!pS || pS.length !== L) continue;
          dS = detectCombo(pS); if (!dS || dS.type !== 'seq') continue;
          nSeq++; scS = residualStruct(hand, pS);
          if (scS > best) { best = scS; keep = [pS]; }
          else if (scS === best) keep.push(pS);
        }
        if (nSeq >= 2 && keep.length && keep.length < nSeq) leg = keep;
      }
      leg = orderLegals(leg, state, myIdx);
    }
