...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      leg = mergedBR;
      // W20 p_w20_brflpair_mid: gold low-pair open — widen FL pair force into mid hand
      // (holdout loss 20270774@0: triple-J over pair-5s). Stricter top<=5.
      var forcedLP = false;
      if (!cur && hand.length >= 7 && oppMinHand(state, myIdx) >= 3) {
        var lps = [], iL;
