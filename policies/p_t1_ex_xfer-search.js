...
(function (root, factory) {
  ...
  /**
   * Transfer-safe T1 com_midclimb_deep (multi-seed force census).
   * Early deep combat vs low top: prefer mid single (ranks 5–8 / 8–J) over
   * ultra min-top when both legal. Gates: hl≥12, omin≥11, curTop≤3, single.
   * SoftN FORBIDDEN. Not exact-byR (anti-overfit).
   */
  function pickComMidClimbDeep(hand, cur, leg, state, cp) {
    if (!cur || cur.type !== 'single') return null;
    if (playIsBomb(cur.cards || [])) return null;
    var handLen = hand.length;
    var omin = oppMinHand(state, cp);
    var curTop = cur.top ? cur.top.rank : (cur.cards && cur.cards[0] ? cur.cards[0].rank : -1);
    if (handLen < 12 || omin < 11 || curTop < 0 || curTop > 3) return null;
    var i, p, minR = 99, mids = [];
    for (i = 0; i < leg.length; i++) {
      p = leg[i];
      if (!p || p.length !== 1) continue;
      if (playIsBomb(p) || playHasTwo(p)) continue;
      if (p[0].rank < minR) minR = p[0].rank;
      // mid: 8,9,T,J (ranks 5–8)
      if (p[0].rank >= 5 && p[0].rank <= 8) mids.push(p);
    }
    if (!mids.length) return null;
    // Only override ultra-min-top (min legal is near floor)
    if (minR > curTop + 2) return null;
    mids.sort(function (a, b) {
      return a[0].rank - b[0].rank || a[0].suit - b[0].suit ||
        expertScore(a, state, cp) - expertScore(b, state, cp);
    });
    // Prefer lowest mid that is not the same as minR (actual climb)
    for (i = 0; i < mids.length; i++) {
      if (mids[i][0].rank > minR) return mids[i];
    }
    return null;
  }

  /**
   * Transfer-safe T1 fl_trash_over_lowseq (gold series trash-first + multi-seed FREE).
   * FREE: when low seq3 (top≤5) is legal and a true singleton trash (rank 0–3, count=1)
   * exists, force trash single. SoftN FORBIDDEN. Not exact-byR.
   */
  function pickFlTrashOverLowSeq(hand, multiOrLeg, state, cp) {
    if (!multiOrLeg || !multiOrLeg.length) return null;
    var handLen = hand.length;
    var omin = oppMinHand(state, cp);
    if (handLen < 9 || omin < 6) return null;
    var byR = {}, i, r, p, c, hasLowSeq = false;
    for (i = 0; i < hand.length; i++) {
      r = hand[i].rank;
      byR[r] = (byR[r] || 0) + 1;
    }
    for (i = 0; i < multiOrLeg.length; i++) {
      ...
      c = detectCombo(p);
      if (!c || c.type !== 'seq') continue;
      var top = c.top ? c.top.rank : -1;
      if (top >= 0 && top <= 5) { hasLowSeq = true; break; }
    }
    if (!hasLowSeq) return null;
    var trash = [];
    for (i = 0; i < multiOrLeg.length; i++) {
      p = multiOrLeg[i];
      if (!p || p.length !== 1) continue;
      if (p[0].rank > 3) continue;
      if ((byR[p[0].rank] || 0) !== 1) continue; // true singleton only
      if (playIsBomb(p) || playHasTwo(p)) continue;
      trash.push(p);
    }
  ...
  function pickFreeLeadHard(leg, state, cp) {
    ...
    if (multi.length) {
      // T1 fl_trash_over_lowseq (transfer-safe)
      var ftls = pickFlTrashOverLowSeq(hand, multi.concat(leg), state, cp);
      if (ftls) return ftls;
  ...
  function searchMove(state, myIdx, opts) {
    ...
    // T1 transfer-safe fl_trash_over_lowseq (gold + multi-seed FREE)
    if (!cur) {
      var trashLowSeqRoot = pickFlTrashOverLowSeq(hand, legals, state, myIdx);
      if (trashLowSeqRoot) {
        return { play: trashLowSeqRoot, stats: { mode: 'fl-trash-over-lowseq-hard', via: 'search-root' } };
      }
    }
    // T1 transfer-safe com_midclimb_deep (multi-seed combat census)
    if (cur && cur.type === 'single') {
      var midClimbDeepRoot = pickComMidClimbDeep(hand, cur, legals, state, myIdx);
      if (midClimbDeepRoot) {
        return { play: midClimbDeepRoot, stats: { mode: 'com-midclimb-deep-hard', via: 'search-root' } };
      }
    }
  ...
  return {
    ...
    pickComMidClimbDeep: pickComMidClimbDeep,
    pickFlTrashOverLowSeq: pickFlTrashOverLowSeq,
