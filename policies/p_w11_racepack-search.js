...
(function (root, factory) {
  ...
  function expertPolicy(state, cp) {
    ...
    // + g2hi
    if (cheap.length && cur.type === 'single' && curTop >= 9 && twoSingles.length) {
      var bestC = orderLegals(cheap, state, cp)[0];
      if (bestC && bestC.length === 1 && bestC[0].rank >= 9) {
        var br = bestC[0].rank, byR = {}, hi, t, chn = 1;
        for (hi = 0; hi < hand.length; hi++) byR[hand[hi].rank] = (byR[hand[hi].rank] || 0) + 1;
        var smash = (byR[br] || 0) >= 2;
        if (!smash) {
          for (t = br - 1; byR[t]; t--) chn++;
          for (t = br + 1; byR[t]; t++) chn++;
          smash = chn >= 3;
        }
        if (smash) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      leg = mergedBR;
      // W10 brflonly: if deep low pairs exist, ONLY score those pairs
      ...
      if (!cur) {
        var oF = oppMinHand(state, myIdx);
        if (oF <= 2) {
          var mOnly = leg.filter(function (p) {
            return p && p.length >= 2 && !playIsExpensive(p);
          });
          if (mOnly.length) leg = mOnly;
        }
      }
    } else {
      ...
      if (ch.length) leg = ch;
      leg = orderLegals(leg, state, myIdx);
      // p_w11_br2race: short-opp combat — force 2 + min-gap only
      if (cur && cur.type === 'single' && cur.top) {
        var oR = oppMinHand(state, myIdx);
        if (oR <= 2) {
          var full = getLegalPlays(hand, cur, state.players[myIdx].passed,
            state.isFirstLead, state.firstLeadCard);
          var twos = [], g2 = [], cT = cur.top.rank, i;
          for (i = 0; i < full.length; i++) {
            if (full[i] && full[i].length === 1) {
              if (full[i][0].rank === 12) twos.push(full[i]);
              else {
                var g = full[i][0].rank - cT;
                if (g >= 1 && g <= 2) g2.push(full[i]);
              }
            }
          }
          if (twos.length || g2.length) {
            leg = twos.concat(g2);
          }
        }
      }
