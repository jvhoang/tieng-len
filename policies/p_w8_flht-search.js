...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      leg = mergedBR;
      // W8A BR_DROP_HIGH_MULTI_IF_TRASH
      if (!cur) {
        var infoT = analyzeHand(hand);
        var hasTrash = false, iT;
        for (iT = 0; iT < leg.length; iT++) {
          if (isTrashSinglePlay(leg[iT], infoT)) { hasTrash = true; break; }
        }
        if (hasTrash && infoT.hasControl && hand.length >= 8 &&
            (infoT.twos >= 1 || infoT.control >= 2)) {
          var anyHigh = false;
          for (iT = 0; iT < leg.length; iT++) {
            if (leg[iT].length >= 2 && !playIsExpensive(leg[iT]) && topRank(leg[iT]) > 6) {
              anyHigh = true; break;
            }
          }
          if (anyHigh) {
            leg = leg.filter(function (p) {
              if (!p || p.length < 2 || playIsExpensive(p)) return true;
              return topRank(p) <= 6;
            });
          }
        }
