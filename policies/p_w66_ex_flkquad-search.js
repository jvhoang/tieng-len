...
(function (root, factory) {
  ...
  /** W66 fl_kquad: FREE force King-quad bomb (A convert 20330612@1 s8).
   *  Base trash single 6C → force KKKK. Gates: FREE, handLen===9, omin===10, twos===0,
   *  byR[10]===4 (quad K), pick expertScore-min among quad rank===K. SoftN FORBIDDEN. */
  function pickFlKQuad(hand, multiOrLeg, state, cp) {
    if (!multiOrLeg || !multiOrLeg.length) return null;
    var handLen = hand.length;
    var omin = oppMinHand(state, cp);
    if (handLen !== 9 || omin !== 10) return null;
    var info = analyzeHand(hand);
    if (info.twos !== 0) return null;
    var byR = {}, i, p, c, r;
    for (i = 0; i < hand.length; i++) {
    ...
    for (i = 0; i < multiOrLeg.length; i++) {
      p = multiOrLeg[i];
      if (!p || p.length !== 4 || playHasTwo(p)) continue;
      c = detectCombo(p);
      // bomb or quad-same-rank
      if (!c) continue;
      if (c.type !== 'bomb' && c.type !== 'quad' && !(p.every(function (x) { return x.rank === 10; }))) {
        // accept any 4-card all-K
        if (!p.every(function (x) { return x.rank === 10; })) continue;
      }
      if (!p.every(function (x) { return x.rank === 10; })) continue;
      quads.push(p);
    }
  ...
  function pickFreeLeadHard(leg, state, cp) {
    ...
    if (multi.length) {
      // W66 fl_kquad FIRST: King-quad bomb beats weak-multi trash early-return
      var fkqEarly = pickFlKQuad(hand, multi.concat(leg), state, cp);
      if (fkqEarly) return fkqEarly;
      ...
      if (fp6) return fp6;
      // W66 fl_kquad: FREE force King-quad bomb before trash single
      var fkq = pickFlKQuad(hand, multi.length ? multi.concat(leg) : leg, state, cp);
      if (fkq) return fkq;
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      var qpcBR = (!sbc0BR && !meBR && !euBR && !ajuBR) ? pickComQPairClimb(hand, cur, leg, state, myIdx) : null;
      var shBR = (!sbc0BR && !meBR && !euBR && !ajuBR && !qpcBR) ? pickSeqHi(hand, cur, leg, state, myIdx) : null;
  ...
  function searchMove(state, myIdx, opts) {
    ...
    // W66 fl_kquad hard FREE
    if (!cur) {
      var kquadRoot = pickFlKQuad(hand, legals, state, myIdx);
      if (kquadRoot) {
        return { play: kquadRoot, stats: { mode: 'fl-kquad-hard', via: 'search-root' } };
      }
    }
