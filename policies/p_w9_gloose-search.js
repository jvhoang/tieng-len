...
(function (root, factory) {
  ...
  function orderLegals(legals, state, myIdx) {
    var hand = state.players[myIdx].hand;
    var cur = state.currentCombo;
    // p_w9_gloose: sbc-first among singles only when true loose (sbc===0 non-2) exists
    var hasLoose = false;
    if (cur && cur.type === 'single') {
      for (var li = 0; li < legals.length; li++) {
        var lp = legals[li];
        if (lp && lp.length === 1 && lp[0].rank !== 12 && structureBreakCost(hand, lp) === 0) {
          hasLoose = true; break;
        }
      }
    }
    return legals.slice().sort(function (a, b) {
      if (hasLoose && a && b && a.length === 1 && b.length === 1) {
