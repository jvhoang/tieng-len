...
(function (root, factory) {
  ...
  function expertPolicy(state, cp) {
    ...
    if (cheap.length && cur.type === 'single' && curTop >= 8 && twoSingles.length) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      leg = mergedBR;
      // BR free-lead force low multi only
      if (!cur && hand.length >= 11) {
        var lps = [];
        for (var iL = 0; iL < leg.length; iL++) {
          if (leg[iL] && leg[iL].length === 2 && topRank(leg[iL]) <= 6 && structureBreakCost(hand, leg[iL]) < 8) lps.push(leg[iL]);
        }
