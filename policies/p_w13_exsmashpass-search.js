...
(function (root, factory) {
  ...
  function expertPolicy(state, cp) {
    ...
    if (cheap.length) {
      // W13 p_w13_exsmashpass: residual smash multi/pair → pass
      if (
        cur.type !== 'single' &&
        !playIsBomb(cur.cards || []) &&
        handLen >= 9 &&
        ...
        var pre = residualKeys(hand, null);
        var allSmash = true, bestMR = -1, ci, sbcI, mrI;
        for (ci = 0; ci < cheap.length; ci++) {
          sbcI = structureBreakCost(hand, cheap[ci]);
          mrI = residualKeys(hand, cheap[ci]).maxRun;
          if (sbcI < 8) allSmash = false;
          if (mrI > bestMR) bestMR = mrI;
        }
        if (allSmash || bestMR < pre.maxRun) return { pass: true };
      }
