...
(function (root, factory) {
  ...
  /** W30 egpack: omin≤2 race package pool (gold 0532) — BR strip required */
  ...
  function enforcePolicyGuards(state, myIdx, proposed) {
    ...
    // W30 egpack guards: keep race package when omin≤2
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      if (ch.length) leg = ch;
      // W29 mulowg FIRST (convert 20270774), then W30 egpack omin≤2 strip
      var mulowApplied = false;
      if (
        cur &&
        cur.type !== 'single' &&
        !playIsBomb(cur.cards || []) &&
      ...
      if (!mulowApplied) {
        ...
        if (raceBR && raceBR.length) leg = raceBR;
        else leg = orderLegals(leg, state, myIdx);
      }
    }
