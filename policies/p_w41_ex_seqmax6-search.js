...
(function (root, factory) {
  ...
  /** W41 fl_seqmax6: BR-only FREE lengthen when the BR multi pool would lead with seq5
   *  and a cheap seq6 exists (maxSeq===6, not flshort≥7). Dual-safe CF B 20400424@0.
   *  Expert unchanged. Tight gate: do NOT override BR seq3/pair wins (A reverse 20410396). */
  function flSeqMax6Cands(hand, multiOrLeg, state, cp) {
    if (!multiOrLeg || !multiOrLeg.length) return [];
    var handLen = hand.length;
    var omin = oppMinHand(state, cp);
    if (handLen < 11 || omin < 4) return [];
    var maxSeq = 0, sixes = [], fives = 0, i, p, c;
    for (i = 0; i < multiOrLeg.length; i++) {
      p = multiOrLeg[i];
      if (!p || playIsExpensive(p) || playHasTwo(p) || playIsBomb(p)) continue;
      c = detectCombo(p);
      ...
      if (p.length === 6) sixes.push(p);
      if (p.length === 5) fives++;
    }
    if (maxSeq >= 7 || maxSeq < 6 || !sixes.length || !fives) return [];
  ...
  /** True if BR would currently lead with a cheap seq5 from leg. */
  function brLeadsWithSeq5(leg, state, myIdx) {
    if (!leg || !leg.length) return false;
    var ordered = orderLegals(leg, state, myIdx);
    if (!ordered.length) return false;
    var top = ordered[0];
    var c = detectCombo(top);
    return !!(c && c.type === 'seq' && top.length === 5 && !playIsExpensive(top));
  }
  ...
  function pickFreeLeadHard(leg, state, cp) {
    ...
    if (multi.length) {
      ...
      if (fls) return fls;
      // W41 fl_seqmax6: expert unchanged (BR-only lever — reverse if hard-forced)
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      // W41 fl_seqmax6 BR: AFTER multi-only / flvol assembly — only if BR would lead seq5,
      // replace pool with seq6 cands. Protects A seats where BR correctly leads seq3.
      if (!cur && !(shortBR && shortBR.length)) {
        var seq6BR = flSeqMax6Cands(hand, fullMultiBR.length ? fullMultiBR : leg, state, myIdx);
        if (seq6BR.length && brLeadsWithSeq5(leg, state, myIdx)) {
          leg = seq6BR;
          forcedLP = false;
        }
      }
