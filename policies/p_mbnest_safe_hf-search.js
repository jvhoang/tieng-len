    ...
    for (var ai = 0; ai < actions.length; ai++) {
      ...
      for (var t = 0; t < nTry; t++) {
        ...
        while (!s.roundOver && steps < 200) {
          ...
          // SAFE_NEST: combat nest hand<=9; free-lead nest only midgame hand 8-11
          var dec;
          if (cp === myIdx) {
            var hSelf = s.players[myIdx].hand.length;
            var curSelf = s.currentCombo;
            if (strongSelf) dec = shallowSelfPick(s, myIdx);
            else if (curSelf && hSelf <= 9) dec = shallowSelfPick(s, myIdx);
            else if (!curSelf && hSelf >= 8 && hSelf <= 11) dec = shallowSelfPick(s, myIdx);
