...
(function (root, factory) {
  ...
  function expertPolicy(state, cp) {
    ...
    // p_pass probe: H_pass_E_play CF — gated mid-combat structure-preserving pass.
    // Deep hand + opp not short + mid pair/triple + cheapest answer shreds structure → fold
    // (pass is legal while combating; do not re-enter if already passed — leg would be empty).
    if (
      handLen >= 9 &&
      omin >= 5 &&
      ...
      var cheapForPass = cheapLegals(leg);
      var answerPool = cheapForPass.length ? cheapForPass : leg;
      var cheapestAns = orderLegals(answerPool, state, cp)[0];
      if (cheapestAns && structureBreakCost(hand, cheapestAns) >= 12) {
        return { pass: true };
      }
    }
