...
(function (root, factory) {
  ...
  /** T2a fl_trash_over_lowseq: FREE singleton trash (0-3) over low seq3. Transfer-safe attempt. */
  function pickFlTrashOverLowSeq(hand, multiOrLeg, state, cp) {
    if (!multiOrLeg || !multiOrLeg.length) return null;
    var handLen = hand.length;
    var omin = oppMinHand(state, cp);
    if (handLen < 10 || omin < 8) return null;
    ...
    for (i = 0; i < multiOrLeg.length; i++) {
      ...
      if (top >= 0 && top <= 4) { hasLowSeq = true; break; }
    }
    ...
    for (i = 0; i < multiOrLeg.length; i++) {
      ...
      if ((byR[p[0].rank] || 0) !== 1) continue;
      if (playIsBomb(p) || playHasTwo(p)) continue;
      trash.push(p);
    }
    if (!trash.length) return null;
    trash.sort(function (a, b) { return a[0].rank - b[0].rank || a[0].suit - b[0].suit; });
    return trash[0];
  }
  ...
  function searchMove(state, myIdx, opts) {
    ...
    // T2a fl_trash_over_lowseq hard FREE
    if (!cur) {
