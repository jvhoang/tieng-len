...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    for (var ai = 0; ai < actions.length; ai++) {
      ...
      for (var t = 0; t < nTry; t++) {
        ...
        if (u >= 0.99) wins += 1;
        else if (!s.roundOver) wins += 0.40 * leafEval2p(s, myIdx);
      }
      var rate = wins / nTry;
  ...
  function leafEval2p(state, myIdx) {
    ...
    // W10 brres residual relative
    var byM = info.byRank || {}, byO = oinfo.byRank || {};
    var pM = 0, pO = 0, rM = 0, rO = 0, run = 0, rr;
    for (rr = 0; rr <= 11; rr++) {
      if ((byM[rr] || 0) >= 2) pM++;
      if ((byO[rr] || 0) >= 2) pO++;
      if ((byM[rr] || 0) > 0) { run++; if (run > rM) rM = run; } else run = 0;
    }
    run = 0;
    for (rr = 0; rr <= 11; rr++) {
      if ((byO[rr] || 0) > 0) { run++; if (run > rO) rO = run; } else run = 0;
    }
    e += (pM - pO) * 0.018;
    e += (Math.min(5, rM) - Math.min(5, rO)) * 0.01;
    // Lead is valuable
