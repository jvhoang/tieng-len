...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      if (ch.length) leg = ch;
      leg = orderLegals(leg, state, myIdx);
      // W8C BR_TWO_WHEN_ALL_SMASH
      if (cur && cur.type === 'single' && cur.top && ch && ch.length) {
        var cT = cur.top.rank;
        if (cT >= 7 && cT <= 11) {
          var byR = {}, hi;
          for (hi = 0; hi < hand.length; hi++) byR[hand[hi].rank] = (byR[hand[hi].rank] || 0) + 1;
          function smashSing(p) {
            if (!p || p.length !== 1) return false;
            var r = p[0].rank, chain = 1, t;
            if ((byR[r] || 0) >= 2) return true;
            for (t = r - 1; byR[t]; t--) chain++;
            for (t = r + 1; byR[t]; t++) chain++;
            return chain >= 3;
          }
          var allS = true, ci;
          for (ci = 0; ci < ch.length; ci++) {
            if (ch[ci].length !== 1 || !smashSing(ch[ci])) { allS = false; break; }
          }
          if (allS) {
            var full2 = getLegalPlays(hand, cur, state.players[myIdx].passed,
              state.isFirstLead, state.firstLeadCard);
            var tw = [];
            for (ci = 0; ci < full2.length; ci++) {
              if (full2[ci].length === 1 && full2[ci][0].rank === 12) tw.push(full2[ci]);
            }
            if (tw.length) {
              leg = leg.filter(function (p) { return !(p && p.length === 1); });
              tw.sort(function (a, b) { return a[0].suit - b[0].suit; });
              leg = tw.concat(leg);
            }
          }
        }
