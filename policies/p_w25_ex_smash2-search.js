...
(function (root, factory) {
  ...
  function expertPolicy(state, cp) {
    ...
    // W25 p_w25_ex_smash2: gold 0513/0525/0544 — 2 for control when only structure-breaks
    // remain among cheap single answers (not SoftN; not BR cand; not W16 pass).
    // Stacks on climbtax. Fires midgame single faces when min-beat would smash pairs/runs.
    if (
      cur.type === 'single' &&
      twoSingles.length &&
      handLen >= 5 &&
      ...
      var cheapS = cheapLegals(leg);
      var singles = [], iS, pS, allBrk = true, nS = 0;
      for (iS = 0; iS < cheapS.length; iS++) {
        pS = cheapS[iS];
        if (!pS || pS.length !== 1) continue;
        if (pS[0].rank === 12) continue; // exclude 2s from "only break" test
        nS++;
        if (structureBreakCost(hand, pS) < 8) allBrk = false;
      }
      if (nS >= 1 && allBrk) {
        twoSingles.sort(function (a, b) { return a[0].suit - b[0].suit; });
        return { play: twoSingles[0] };
