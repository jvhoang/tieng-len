...
(function (root, factory) {
  ...
  /** W73 seqrespair: combat same-len seq residual pair-keep (gold 0503/0519).
   *  Among same-length cheap seq answers, pick unique max residual pairR
   *  when that disagrees with orderLegals min-score. SoftN FORBIDDEN. */
  function residualPairRun(hand, play) {
  ...
  function pickSeqResPair(hand, cur, leg, state, cp) {
    if (!cur || cur.type !== 'seq') return null;
    if (playIsBomb(cur.cards || [])) return null;
    var curTop = cur.top ? cur.top.rank : 0;
    if (curTop >= 11) return null;
    var handLen = hand.length, omin = oppMinHand(state, cp);
    if (handLen < 7 || omin < 3) return null;
    var curLen = (cur.cards && cur.cards.length) || 0;
    if (curLen < 3) return null;
    var pool = [], i, p, c;
    for (i = 0; i < leg.length; i++) {
      p = leg[i];
      if (!p || p.length !== curLen) continue;
      if (playHasTwo(p) || playIsBomb(p) || playIsExpensive(p)) continue;
      c = detectCombo(p);
      if (c && c.type === 'seq') pool.push(p);
    }
    if (pool.length < 2) return null;
    var ranked = pool.slice().sort(function (a, b) {
      var ra = residualPairRun(hand, a), rb = residualPairRun(hand, b);
      if (ra.pairR !== rb.pairR) return rb.pairR - ra.pairR;
      if (ra.maxRun !== rb.maxRun) return rb.maxRun - ra.maxRun;
      return expertScore(a, state, cp) - expertScore(b, state, cp);
    });
    var w = ranked[0], r2 = ranked[1];
    var rw = residualPairRun(hand, w), rr = residualPairRun(hand, r2);
    if (rw.pairR === rr.pairR) return null; // need unique pair keep
    if (rw.pairR <= rr.pairR) return null;
    var def = orderLegals(pool, state, cp)[0];
    if (playSig(w) === playSig(def)) return null;
    return w;
  }
  ...
  function expertPolicy(state, cp) {
    ...
    // W73 seqrespair: same-len seq residual pair-keep
    var srp = pickSeqResPair(hand, cur, leg, state, cp);
    if (srp) return { play: srp };
    ...
    if (cheap.length) {
      ...
      var srpC = pickSeqResPair(hand, cur, cheap, state, cp);
      if (srpC) return { play: srpC };
      if (
        cur &&
        cur.type !== 'single' &&
        !playIsBomb(cur.cards || []) &&
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      var srpBR = (!sbc0BR && !meBR && !euBR && !ajuBR && !qpcBR && !shBR && !shrBR && !shr13BR && !saBR && !s5BR && !smuBR && !phBR) ? pickSeqResPair(hand, cur, leg, state, myIdx) : null;
      if (sbc0BR) {
        leg = [sbc0BR];
      } else if (meBR) {
        leg = [meBR];
