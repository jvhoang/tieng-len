...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      if (ch.length) leg = ch;
      leg = orderLegals(leg, state, myIdx);
      // W12C BR_PAIR_HIGH_STRIP_PASS
      if (cur.type === 'pair' && hand.length >= 9 &&
          oppMinHand(state, myIdx) >= 5 && (cur.top ? cur.top.rank : 0) < 9 &&
          !playIsBomb(cur.cards || [])) {
        var pairsC = [];
        var srcP = ch.length ? ch : leg;
        for (var iP = 0; iP < srcP.length; iP++) {
          if (srcP[iP] && srcP[iP].length === 2) pairsC.push(srcP[iP]);
        }
        if (pairsC.length && pairsC.every(function (p) { return topRank(p) >= 9; })) {
          leg = []; // pass becomes unique scored action
        }
      }
    }
    if (leg.length > maxBranch) leg = leg.slice(0, maxBranch);

    var allowPass = !!cur && (leg.length === 0 || cheapLegals(
