...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    // W16 p_w16_brbestpass: gold 0501 — if BR-best multi is high smash, force pass instead
    var bestPass = false;
    if (cur && cur.type !== 'single' && !playIsBomb(cur.cards || []) &&
        hand.length >= 8 && oppMinHand(state, myIdx) >= 4 &&
        (cur.top ? cur.top.rank : 0) <= 9 && leg.length) {
      var bestM = leg[0];
      var infoB = analyzeHand(hand);
      var canR = (infoB.twos >= 1 || infoB.control >= 2);
      if (!canR) {
        var byB = {}, ib;
        for (ib = 0; ib < hand.length; ib++) byB[hand[ib].rank] = (byB[hand[ib].rank] || 0) + 1;
        var mp = 0, rb;
        for (rb = 6; rb <= 11; rb++) if ((byB[rb] || 0) >= 2) mp++;
        if (mp >= 1) canR = true;
      }
      if (canR && bestM && bestM.length >= 2 &&
          topRank(bestM) >= 9 && structureBreakCost(hand, bestM) >= 8) {
        bestPass = true;
        leg = [];
      }
    }
    ...
    var allowPass = bestPass || (!!cur && cheapLegals(
