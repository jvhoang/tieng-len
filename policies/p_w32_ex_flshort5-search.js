...
(function (root, factory) {
  ...
  /** W31 flvol: FREE low naked pair → volume multi (triple/seq/dseq). CF 20310666/20390451. */
  function flVolPool(hand, multiOrLeg, state, cp) {
    ...
    for (i = 0; i < multiOrLeg.length; i++) {
      ...
      var isVol = (c.type === 'triple' || c.type === 'doubleseq' || c.type === 'quad') ||
        (c.type === 'seq' && p.length >= 3);
      if (isVol && top <= 7 && structureBreakCost(hand, p) < 12) volMulti.push(p);
    }
