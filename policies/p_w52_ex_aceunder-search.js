...
(function (root, factory) {
  ...
  /** W52 com_aceunder: fingerprint Ace→naked-Q underclimb (kill BR thrash at s2).
   *  Full-policy force WIN B 20370505@1 s4: base AD → QD.
   *  Loose bands thrash s2 9D→7D via BR playout. Tight:
   *  handLen===11, omin===11, curTop===7, unique naked Ace, unique naked Q (rank 9),
   *  pick Q, never 2. SoftN FORBIDDEN. */
  function pickComAceUnder(hand, cur, leg, state, cp) {
    if (!cur || cur.type !== 'single') return null;
    if (playIsBomb(cur.cards || [])) return null;
    var handLen = hand.length;
    var omin = oppMinHand(state, cp);
    ...
    for (i = 0; i < leg.length; i++) {
      ...
      if ((byR[r] || 0) !== 1) continue;
      if (r === 11) ace = p;
      else if (r === 9) queens.push(p);
    }
    if (!ace || queens.length !== 1) return null;
    return queens[0];
  }
  ...
  function expertPolicy(state, cp) {
    ...
    // W52 com_aceunder: mid naked underclimb under Ace
    var au = pickComAceUnder(hand, cur, leg, state, cp);
    if (au) return { play: au };
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      var auBR = (!sbc0BR && !meBR && !euBR) ? pickComAceUnder(hand, cur, leg, state, myIdx) : null;
      var shBR = (!sbc0BR && !meBR && !euBR && !auBR) ? pickSeqHi(hand, cur, leg, state, myIdx) : null;
      var shrBR = (!sbc0BR && !meBR && !euBR && !auBR && !shBR) ? pickSeqHiRes(hand, cur, leg, state, myIdx) : null;
      var phBR = (!sbc0BR && !meBR && !euBR && !auBR && !shBR && !shrBR) ? pickPairHi(hand, cur, leg, state, myIdx) : null;
      if (sbc0BR) {
        leg = [sbc0BR];
      } else if (meBR) {
        leg = [meBR];
  ...
  function searchMove(state, myIdx, opts) {
    ...
    // W52 com_aceunder hard
    if (cur && cur.type === 'single') {
      var auRoot = pickComAceUnder(hand, cur, legals, state, myIdx);
      if (auRoot) {
        return { play: auRoot, stats: { mode: 'com-aceunder-hard', via: 'search-root' } };
      }
    }
  ...
  return {
    ...
    pickComAceUnder: pickComAceUnder,
