...
(function (root, factory) {
  ...
  /** T2b com_midclimb_tight: deep early combat, curTop≤1, force rank 6-7 (9-T) over min. */
  function pickComMidClimbTight(hand, cur, leg, state, cp) {
    if (!cur || cur.type !== 'single') return null;
    if (playIsBomb(cur.cards || [])) return null;
    var handLen = hand.length;
    var omin = oppMinHand(state, cp);
    ...
    for (i = 0; i < leg.length; i++) {
      ...
      if (p[0].rank >= 6 && p[0].rank <= 7) mids.push(p); // 9,T only
    }
    if (!mids.length || minR > curTop + 1) return null;
  ...
  function searchMove(state, myIdx, opts) {
    ...
    // T2b com_midclimb_tight hard
    if (cur && cur.type === 'single') {
      var midTightRoot = pickComMidClimbTight(hand, cur, legals, state, myIdx);
      if (midTightRoot) {
        return { play: midTightRoot, stats: { mode: 'com-midclimb-tight-hard', via: 'search-root' } };
      }
    }
  ...
  return {
    ...
    pickComMidClimbTight: pickComMidClimbTight,
