...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      addBR(fl);
      addBR(extraBR);
      // Prefer SHORT multi first (CF multiVsSingle / short-multi) then top
      mergedBR.sort(function (a, b) {
        var ma = a.length >= 2 ? 1 : 0, mb = b.length >= 2 ? 1 : 0;
        if (ma !== mb) return mb - ma; // multi before singles
        if (a.length !== b.length && ma) return a.length - b.length; // short multi first
        return topRank(a) - topRank(b);
      });
      leg = mergedBR;
    } else {
