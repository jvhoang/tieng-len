...
(function (root, factory) {
  ...
  /** true if residual after a is strictly better than after b */
  function residualBetter(hand, a, b) {
    var ra = residualAfter(hand, a), rb = residualAfter(hand, b);
    if (ra.sbc !== rb.sbc) return ra.sbc < rb.sbc;
    if (ra.pairR !== rb.pairR) return ra.pairR > rb.pairR;
    if (ra.maxRun !== rb.maxRun) return ra.maxRun > rb.maxRun;
    return false;
  }
  ...
  function expertPolicy(state, cp) {
    ...
    // W29 p_w29_ex_mulow_rg: min-top multi ONLY when residual after min-top is strictly
    // better than after max-top (convert 20270774, avoid reverse 20460262).
    // SoftN/pass forbidden.
    if (
      cur &&
      cur.type !== 'single' &&
      !playIsBomb(cur.cards || []) &&
      ...
      if (lowM.length >= 2) {
        ...
        var maxM = lowM[lowM.length - 1];
        // re-find max top for residual compare
        var hiM = lowM[0];
        for (iL = 0; iL < lowM.length; iL++) {
          if (topRank(lowM[iL]) >= topRank(hiM)) hiM = lowM[iL];
        }
        if (residualBetter(hand, lowM[0], hiM)) return { play: lowM[0] };
      }
    }
    ...
    if (cheap.length) {
      if (
        ...
        if (cheapLow.length >= 2) {
          ...
          var hiC = cheapLow[0];
          for (iCL = 0; iCL < cheapLow.length; iCL++) {
            if (topRank(cheapLow[iCL]) >= topRank(hiC)) hiC = cheapLow[iCL];
          }
          if (residualBetter(hand, cheapLow[0], hiC)) return { play: cheapLow[0] };
        }
      }
      ...
      if (
        ...
        if (infoBR.twos >= 1 || infoBR.control >= 2) {
          ...
          if (brLow.length >= 2) {
            ...
            // W29: force min-top ONLY when residual after min-top better than max-top
            var hiBR = brLow[0];
            for (var iH = 0; iH < brLow.length; iH++) {
              if (topRank(brLow[iH]) >= topRank(hiBR)) hiBR = brLow[iH];
            }
            if (residualBetter(hand, brLow[0], hiBR)) {
