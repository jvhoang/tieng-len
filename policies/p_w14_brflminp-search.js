...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      leg = mergedBR;
      // W14 p_w14_brflminp: FL pairs only min-top (not top cap) when deep
      if (!cur && hand.length >= 11) {
        var pairs = [], minT = 99, iP;
        for (iP = 0; iP < leg.length; iP++) {
          if (leg[iP] && leg[iP].length === 2 && structureBreakCost(hand, leg[iP]) < 8) {
            pairs.push(leg[iP]);
            var t = topRank(leg[iP]);
            if (t < minT) minT = t;
          }
        }
        if (pairs.length >= 2 && minT < 99) {
          leg = pairs.filter(function (p) { return topRank(p) === minT; });
        } else if (pairs.length === 1 && minT <= 6) {
          leg = pairs; // sole low-ish pair still force
        }
      }
    } else {
