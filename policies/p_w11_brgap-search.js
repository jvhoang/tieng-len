...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      if (ch.length) leg = ch;
      leg = orderLegals(leg, state, myIdx);
      // p_w11_brgap: only min-gap singles when gap<=2 exists
      if (cur && cur.type === 'single' && cur.top && hand.length >= 8) {
        var oG = oppMinHand(state, myIdx);
        if (oG >= 4) {
          var cT = cur.top.rank, g2 = [], iG;
          for (iG = 0; iG < leg.length; iG++) {
            if (leg[iG] && leg[iG].length === 1) {
              var gap = leg[iG][0].rank - cT;
              if (gap >= 1 && gap <= 2) g2.push(leg[iG]);
            }
          }
          if (g2.length) leg = g2;
        }
