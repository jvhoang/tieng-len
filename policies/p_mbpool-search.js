...
(function (root, factory) {
  ...
  function pickFreeLeadHard(leg, state, cp) {
    ...
    if (multi.length) {
      ...
      // Prefer short multi (pair/triple) over long dumps when tops close (CF multiVsSingle)
      var shortM = pool.filter(function (p) { return p.length >= 2 && p.length <= 3; });
      if (shortM.length) pool = shortM;
      pool = pool.slice().sort(function (a, b) {
        var la = a.length, lb = b.length;
        var ta = topRank(a), tb = topRank(b);
        if (la !== lb && Math.abs(ta - tb) <= 2) return la - lb; // short first
