...
(function (root, factory) {
  ...
  // W12B residual pair count after removing play (ranks 0..11)
  function residualPairR(hand, play) {
    var used = Object.create(null), i, r, byR = Object.create(null), pairR = 0;
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      if (ch.length) leg = ch;
      leg = orderLegals(leg, state, myIdx);
      // W12B BR_COMBAT_RESID_KEEP — drop multi cands that lose residual pairs
      if (cur) {
        var maxPR = -1, iR, pr;
        for (iR = 0; iR < leg.length; iR++) {
          if (leg[iR] && leg[iR].length >= 2) {
            pr = residualPairR(hand, leg[iR]);
        ...
        if (maxPR >= 1) {
          leg = leg.filter(function (p) {
            ...
            return residualPairR(hand, p) >= maxPR;
          });
        }
      }
    }
