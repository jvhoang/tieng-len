...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      leg = mergedBR;
      // W10 brflpair: deep free-lead — drop trash singles when low pair exists
      if (!cur && hand.length >= 11) {
        var infoL = analyzeHand(hand);
        var hasLP = false, iL;
        for (iL = 0; iL < leg.length; iL++) {
          if (leg[iL] && leg[iL].length === 2 && topRank(leg[iL]) <= 5 &&
              structureBreakCost(hand, leg[iL]) < 8) { hasLP = true; break; }
        }
        if (hasLP) {
          leg = leg.filter(function (p) {
            if (!p) return true;
            if (p.length === 1 && isTrashSinglePlay(p, infoL)) return false;
            return true;
          });
        }
      }
