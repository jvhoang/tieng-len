...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    var fullLeg = getLegalPlays(hand, cur, state.players[myIdx].passed, state.isFirstLead, state.firstLeadCard);
    var cheapAll = cheapLegals(fullLeg);
    var allowPass = !!cur && cheapAll.length === 0;
    // Gated structure fold: allow pass among cheap when deep + mid multi + all cheap break structure
    if (!allowPass && cur && cur.type !== 'single' && hand.length >= 9 && oppMinHand(state, myIdx) >= 5 && cur.top && cur.top.rank < 9 && cheapAll.length) {
      var allBreak = true;
      for (var ci = 0; ci < cheapAll.length; ci++) {
        if (structureBreakCost(hand, cheapAll[ci]) < 10) { allBreak = false; break; }
      }
      if (allBreak) allowPass = true;
    }
    ...
    for (var ai = 0; ai < actions.length; ai++) {
      ...
      // passTie: mild prior for fold when deep + mid multi (humanPassAltPlay)
      if (act == null && cur && hand.length >= 9 && oppMinHand(root, myIdx) >= 5 && cur.top && cur.top.rank < 9) {
        multiTie += 0.012;
      }
