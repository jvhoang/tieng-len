...
(function (root, factory) {
  ...
  /** W27: midgame single-2 for control when min-beat is high smash (gold 0513/0525/0544) */
  function pickCtrl2Hi(hand, cur, leg, state, cp) {
    if (!cur || cur.type !== 'single') return null;
    var curTop = cur.top ? cur.top.rank : 0;
    if (curTop < 7 || curTop > 10) return null; // Ace+2 owns ≥11; include J faces
    ...
    for (i = 0; i < leg.length; i++) {
      if (leg[i] && leg[i].length === 1 && leg[i][0].rank === 12) twoSingles.push(leg[i]);
    }
    ...
    var minBeat = null, nSing = 0, hasLoose = false, p, sbc;
    for (i = 0; i < cheap.length; i++) {
      p = cheap[i];
      if (!p || p.length !== 1) continue;
      if (p[0].rank === 12) continue;
      nSing++;
      sbc = structureBreakCost(hand, p);
    ...
    // W27 loose mass: high smash threshold relaxed for dual mass (still pair-break class)
    if (mbSbc < 8 || minBeat[0].rank < 9) return null;
  ...
  function expertPolicy(state, cp) {
    ...
    // W27 p_w27_ex_ctrl2hi: 2 for control on high smash (before cheap short-circuit)
    var ctrl2e = pickCtrl2Hi(hand, cur, leg, state, cp);
    if (ctrl2e) return { play: ctrl2e };
  ...
  function enforcePolicyGuards(state, myIdx, proposed) {
    ...
    // W27 ctrl2hi guards: keep 2 for control before cheap smash force
    var gCtrl2 = pickCtrl2Hi(hand, cur, leg, state, myIdx);
    if (gCtrl2) {
      if (proposed && proposed.length === 1 && proposed[0].rank === 12 && isLegalPlay(proposed)) {
        return proposed;
      }
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      var fullCbr = leg.slice();
      var ch = cheapLegals(leg);
      if (ch.length) leg = ch;
      // W27 ctrl2hi: reinject single-2 so BR root can select control reclaim
      var rein2 = pickCtrl2Hi(hand, cur, fullCbr, state, myIdx);
      if (rein2) {
        // reinject 2 alongside cheap so BR can choose control reclaim
        var seen = {}, merged = [rein2], iR, pR, sg;
        seen[playSig(rein2)] = 1;
        for (iR = 0; iR < leg.length; iR++) {
          pR = leg[iR];
          sg = playSig(pR);
          if (!seen[sg]) { seen[sg] = 1; merged.push(pR); }
        }
        leg = merged;
      }
      leg = orderLegals(leg, state, myIdx);
