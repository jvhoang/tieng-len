...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      if (ch.length) leg = ch;
      leg = orderLegals(leg, state, myIdx);
      // W12A BR_PAIRTRIP_MINDROP — pair/triple keep min top only (not singles)
      if (cur && (cur.type === 'pair' || cur.type === 'triple')) {
        var minT = 99, iT, hasVar = false, seenT = {};
        for (iT = 0; iT < leg.length; iT++) {
          if (!leg[iT] || leg[iT].length < 2) continue;
          var t0 = topRank(leg[iT]);
          if (t0 < minT) minT = t0;
          if (seenT[t0]) continue;
          seenT[t0] = 1;
          if (Object.keys(seenT).length > 1) hasVar = true;
        }
        if (hasVar && minT < 99) {
          leg = leg.filter(function (p) {
            return p && p.length >= 2 && topRank(p) === minT;
          });
        }
      }
    }
