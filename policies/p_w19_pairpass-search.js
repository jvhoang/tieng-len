...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    // W19 p_w19_pairpass: gold 0547 — allow PASS mid pair-war when only high pair answers
    if (!allowPass && cur && cur.type === 'pair' && hand.length >= 7 &&
        oppMinHand(state, myIdx) >= 4 && !playIsBomb(cur.cards || [])) {
      var allHi = true, nPr = 0, iP, pP;
      for (iP = 0; iP < leg.length; iP++) {
        pP = leg[iP]; if (!pP || pP.length !== 2) continue;
        nPr++;
        if (topRank(pP) < 10) { allHi = false; break; }
      }
      if (nPr >= 1 && allHi) allowPass = true;
    }
