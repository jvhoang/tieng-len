...
(function (root, factory) {
  ...
  function shallowSelfPick(state, myIdx) {
    ...
    // W7c: try pass when deep mid multi and all cheap smash
    if (cur && cur.type !== 'single' && !playIsBomb(cur.cards || []) &&
        hand.length >= 9 && oppMinHand(state, myIdx) >= 5 && (cur.top ? cur.top.rank : 0) < 9) {
      var chP = cheapLegals(getLegalPlays(hand, cur, state.players[myIdx].passed, state.isFirstLead, state.firstLeadCard));
      if (chP.length && chP.every(function (p) { return structureBreakCost(hand, p) >= 10; })) {
        var nPass = passFast(state, myIdx);
        nPass.isFirstLead = false;
        if (exploitPlayoutLeaf(nPass, myIdx, 200) >= 0.55) return { pass: true };
      }
    }
    for (i = 0; i < leg.length; i++) {
