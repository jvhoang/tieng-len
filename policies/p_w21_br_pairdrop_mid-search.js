...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      if (ch.length) leg = ch;
      // W21 p_w21_br_pairdrop_mid: gold loose>pair-break, mid holdout gates
      var ominA = oppMinHand(state, myIdx), ctA = cur.top ? cur.top.rank : 99;
      if (cur && cur.type === 'single' && hand.length >= 6 && hand.length <= 11
          && ominA >= 5 && ctA <= 8) {
        var hasLoose = false, iA, pA;
        for (iA = 0; iA < leg.length; iA++) {
          pA = leg[iA];
          if (pA && pA.length === 1 && structureBreakCost(hand, pA) < 8) hasLoose = true;
        }
        if (hasLoose) leg = leg.filter(function (p) {
          return !p || p.length !== 1 || structureBreakCost(hand, p) < 12;
        });
      }
      leg = orderLegals(leg, state, myIdx);
    }
