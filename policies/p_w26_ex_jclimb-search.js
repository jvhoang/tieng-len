  ...
  function expertScore(play, state, myIdx) {
    ...
    if (!cur) {
      ...
      if (cur.type === 'single' && com.type === 'single') {
        ...
        // W26 p_w26_ex_jclimb: extend climb tax to Jack (rank 9) — climbtax missed
        // JH start of JH→QS→KS freezes (20300693 class). Gold residual structure.
        if (!usesTwo && gap >= 2 && com.top.rank === 9 && curTop <= 6 && handLen >= 7) {
          if (info.twos >= 1 || info.control >= 2 || info.trashCount >= 1) score += 28 + gap * 2;
        }
        // Mild tax on high multi over low faces when residual 2s (seq/pair burn)
        if (
          !usesTwo &&
          (com.type === 'pair' || com.type === 'seq' || com.type === 'triple') &&
          com.top.rank >= 10 &&
          curTop <= 6 &&
          handLen >= 7 &&
          (info.twos >= 1 || info.control >= 2)
        ) {
          score += 20 + (com.top.rank - curTop);
        }
      }
