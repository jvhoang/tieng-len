...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    for (var ai = 0; ai < actions.length; ai++) {
      ...
      // W3D COMBAT_RES_SOFT: residual structure soft-tie in combat
      if (act && cur) {
        var rk = residualMultiKeys(hand, act);
        multiTie += 0.004 * rk.pairR + 0.002 * rk.maxRun;
        multiTie -= Math.min(0.015, structureBreakCost(hand, act) * 0.0008);
      }
