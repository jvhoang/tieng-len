...
(function (root, factory) {
  ...
  function residualKeys(hand, play) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    // W13 p_w13_brresband: residual hard among near-rate band (combat)
    if (cur && details.length) {
      var nTryB = trials, band = 1 / Math.max(1, nTryB);
      var bestR = details[0].rate;
      var pool = details.filter(function (d) { return d.rate + 1e-9 >= bestR - band; });
      function actOf(sig) {
        if (sig === 'PASS') return null;
        for (var ai2 = 0; ai2 < actions.length; ai2++)
          if (playSig(actions[ai2]) === sig) return actions[ai2];
        return bestPlay;
      }
      pool.sort(function (da, db) {
        var a = actOf(da.sig), b = actOf(db.sig);
        if (!a && b) return 1; if (a && !b) return -1; if (!a && !b) return 0;
        var ka = residualKeys(hand, a), kb = residualKeys(hand, b);
        if (kb.maxRun !== ka.maxRun) return kb.maxRun - ka.maxRun;
        if (kb.pairR !== ka.pairR) return kb.pairR - ka.pairR;
        var sa = structureBreakCost(hand, a), sb = structureBreakCost(hand, b);
        if (sa !== sb) return sa - sb;
        return topRank(a) - topRank(b);
      });
      bestPlay = actOf(pool[0].sig);
    }
    return {
      play: bestPlay,
      stats: {
