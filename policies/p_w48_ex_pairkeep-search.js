...
(function (root, factory) {
  ...
  /** W48 com_pairkeep: combat single — unique max naked under Ace-pair break.
   *  Dual-safe CF B 20470235@0 s4 under v95: base AC (breaks AA) → mid naked WIN.
   *  First draft min-naked thrash (6S + 20370505@1). Tightened:
   *  handLen 9–11, omin 8–10, curTop≤2, pair-break must include Ace (rank 11),
   *  pick unique max-rank naked with rank < 11 and rank ≥ 7, never 2.
   *  Orthogonal to sbc0. SoftN FORBIDDEN. */
  function pickComPairKeep(hand, cur, leg, state, cp) {
    if (!cur || cur.type !== 'single') return null;
    if (playIsBomb(cur.cards || [])) return null;
    var handLen = hand.length;
    var omin = oppMinHand(state, cp);
    var curTop = cur.top ? cur.top.rank : 0;
    // Convert CF: hl10 omin9 curTop1. Thrash 20370505 hl12 omin12.
    if (handLen < 9 || handLen > 11) return null;
    if (omin < 8 || omin > 10) return null;
    ...
    // Must hold Ace pair (the structure we refuse to break)
    if ((byR[11] || 0) < 2) return null;
    var naked = [], hasAceBreak = false;
    for (i = 0; i < leg.length; i++) {
      p = leg[i];
      if (!p || p.length !== 1) continue;
      r = p[0].rank;
      if (r === 12) continue; // never auto-force 2
      if (r === 11 && (byR[11] || 0) >= 2) hasAceBreak = true;
      if ((byR[r] || 0) === 1 && r >= 7 && r < 11) {
        naked.push({ p: p, rank: r });
      }
    }
    if (!hasAceBreak || naked.length < 1) return null;
    // unique max-rank naked in [7..10]
    naked.sort(function (a, b) {
      if (a.rank !== b.rank) return b.rank - a.rank;
      return a.p[0].suit - b.p[0].suit;
    });
    // Prefer unique Jack (CF convert JC). Max-Q dual-null under full policy.
    var jacks = naked.filter(function (x) { return x.rank === 9; });
    if (jacks.length !== 1) return null;
    return jacks[0].p;
  }
  ...
  function expertPolicy(state, cp) {
    ...
    // W48 com_pairkeep: naked single over pair-break (after sbc0; orthogonal)
    var pk = pickComPairKeep(hand, cur, leg, state, cp);
    if (pk) return { play: pk };
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      // W48 com_pairkeep BR: naked single over pair-break
      var pkBR = pickComPairKeep(hand, cur, leg, state, myIdx);
      if (pkBR) {
        leg = [pkBR];
      } else
  ...
  function searchMove(state, myIdx, opts) {
    ...
    // W48 com_pairkeep hard (naked over pair-break)
    if (cur && cur.type === 'single') {
      var pkRoot = pickComPairKeep(hand, cur, legals, state, myIdx);
      if (pkRoot) {
        return { play: pkRoot, stats: { mode: 'com-pairkeep-hard', via: 'search-root' } };
      }
    }
  ...
  return {
    ...
    pickComPairKeep: pickComPairKeep,
