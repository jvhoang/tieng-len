...
(function (root, factory) {
  ...
  function structureBreakCost(hand, play) {
    ...
    for (var k = 0; k < keys.length; k++) {
      ...
      // gold-surgical inflate: pair smash scale
    ...
    // gold-surgical: stronger sequence / multi-combo break (≥3-chain aware)
    if (play.length === 1) {
      var pr0 = play[0].rank;
      var nbrL = byRank[pr0 - 1] || 0;
      var nbrR = byRank[pr0 + 1] || 0;
      var left2 = byRank[pr0 - 2] || 0;
      var right2 = byRank[pr0 + 2] || 0;
  ...
  function orderLegals(legals, state, myIdx) {
    ...
    return legals.slice().sort(function (a, b) {
      // gold-surgical: combat structure-first (break cost → min top → expertScore)
      if (!a && b) return 1;
      if (a && !b) return -1;
      if (!a && !b) return 0;
      if (cur) {
        var ca = structureBreakCost(hand, a);
        var cb = structureBreakCost(hand, b);
        if (ca !== cb) return ca - cb;
        function top(p) {
          var t = -1;
          for (var i = 0; i < p.length; i++) if (p[i].rank > t) t = p[i].rank;
          return t;
        }
  ...
  function pickFreeLeadHard(leg, state, cp) {
    ...
    if (multi.length) {
      ...
      // FL hybrid trash: handLen>=9 && multi top>6 → lowest trash single
      if (
        trashPlays.length >= 1 &&
      ...
      // Residual: hybrid only when exploit dual-self scores hybrid mode
      if (
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    for (var ai = 0; ai < actions.length; ai++) {
      ...
      // gold-surgical combat BR soft-tie: residual structure among near-equal rates
      if (act && cur) {
