...
(function (root, factory) {
  ...
  function pickFreeLeadHard(leg, state, cp) {
    ...
    if (multi.length) {
      ...
      // v7.5 multi-always core; ENS1: prefer length 2-3 over 4+ when tops within 2
      ...
      pool = pool.slice().sort(function (a, b) {
        ...
        // ENS1 short-multi: prefer pair/triple (2-3) over long multi (4+) when tops close
        if (Math.abs(ta - tb) <= 2) {
          var shortA = (la >= 2 && la <= 3) ? 1 : 0;
          var shortB = (lb >= 2 && lb <= 3) ? 1 : 0;
          if (shortA !== shortB) return shortB - shortA;
        }
        // residual mild length preference among same short/long band when tops within 1
        if (la !== lb && Math.abs(ta - tb) <= 1) return lb - la;
        return expertScore(a, state, cp) - expertScore(b, state, cp);
      });
      var multiPick = pool[0];
      // ENS1 / FL_HYBRID_STRONG: human trash-first when deep + control and multi top high
      // (from p_flstrong: handLen>=9 and multi top>6 or long multi)
      if (
        trashPlays.length >= 1 &&
      ...
      // Hybrid trash only when exploit dual-self scores hybrid mode (ladder residual path).
      if (
