...
(function (root, factory) {
  ...
  function residualMaxRun(hand, play) {
    var used = Object.create(null), i, r, byR = Object.create(null), maxRun = 0, run = 0;
    ...
    for (r = 0; r <= 11; r++) {
      if ((byR[r] || 0) >= 1) { run++; if (run > maxRun) maxRun = run; }
      else run = 0;
    }
    return maxRun;
  }
  ...
  function expertPolicy(state, cp) {
    ...
    // W12 gr2: admit single-2 when every cheap single drops residual maxRun
    if (cheap.length && cur.type === 'single' && curTop >= 9 && twoSingles.length) {
      var preRun = residualMaxRun(hand, []);
      var allDrop = true;
      var iC;
      for (iC = 0; iC < cheap.length; iC++) {
        if (cheap[iC].length !== 1) { allDrop = false; break; }
        if (residualMaxRun(hand, cheap[iC]) >= preRun) { allDrop = false; break; }
      }
      if (allDrop) {
        var pool2 = cheap.concat(twoSingles);
        return { play: orderLegals(pool2, state, cp)[0] };
      }
    }
