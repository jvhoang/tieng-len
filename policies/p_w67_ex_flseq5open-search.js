...
(function (root, factory) {
  ...
  /** W67 fl_seq5open: FREE opening max-length seq with single-2 + AA (A convert 20360531@0).
   *  Base short 345 → force max seq (5 or 6). Disjoint seqopen (requires twos≥2).
  ...
  function pickFreeLeadHard(leg, state, cp) {
    ...
    if (multi.length) {
      // W67 fl_seq5open FIRST: single-2+AA maxL5 open (before weak-multi trash)
      var fs5Early = pickFlSeq5Open(hand, multi.concat(leg), state, cp);
      if (fs5Early) return fs5Early;
