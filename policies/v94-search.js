...
(function (root, factory) {
  ...
  function expertPolicy(state, cp) {
    ...
    // V94_TWO_OMIN2: one-axis only — fire 2 vs mid tops when opp very short (omin≤2).
    // Targets residual short multi-tempo (e.g. 20549928 class); dual-asymmetric vs freeze v93.
    if (
      cur.type === 'single' &&
      curTop >= 7 &&
      curTop <= 10 &&
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    for (var ai = 0; ai < actions.length; ai++) {
      ...
      if (act && act.length >= 2 && !cur) {
        ...
        // V93_RESIDUAL_MT: BR-path only (freeze seat has bestResponse:false).
        // Prefer free-lead multi that leaves residual pair structure (not blanket 0.008).
        var handMt = root.players[myIdx].hand;
        var usedMt = Object.create(null);
        var uiMt;
        for (uiMt = 0; uiMt < act.length; uiMt++) {
          usedMt[act[uiMt].rank * 4 + act[uiMt].suit] = 1;
        }
        var byRMt = Object.create(null);
        var hiMt;
        for (hiMt = 0; hiMt < handMt.length; hiMt++) {
          if (!usedMt[handMt[hiMt].rank * 4 + handMt[hiMt].suit]) {
            byRMt[handMt[hiMt].rank] = (byRMt[handMt[hiMt].rank] || 0) + 1;
          }
        }
        var pairRanksMt = 0;
        var rkMt;
        for (rkMt in byRMt) {
          if (byRMt[rkMt] >= 2) pairRanksMt++;
        }
        if (pairRanksMt >= 1) multiTie += 0.004;
        if (pairRanksMt >= 2) multiTie += 0.002;
      }
  ...
  function exploitMove(state, myIdx, opts) {
    ...
    // STACK: enable softSamples (bench-ladder sets softSamples:0 which would otherwise skip)
    if (!primary.anyWin && state.players.length === 2 && opts.softSamples !== false) {
      var softN = (opts.softSamples != null && opts.softSamples > 0) ? opts.softSamples : 10; // STACK default 10
  ...
  function leafEval2p(state, myIdx) {
    ...
    // Base: card race (v9.2 race+ — dual-neutral at 0.68, keep as base)
    var e = 0.5 + (oppLen - myLen) * 0.068;
    e += (info.twos - oinfo.twos) * 0.065;
    e += (info.control - oinfo.control) * 0.032;
    e += (oinfo.trashCount - info.trashCount) * 0.020;
    // Lead is valuable
    if (state.currentCombo == null && state.currentPlayer === myIdx) e += 0.05;
    if (state.currentCombo == null && state.currentPlayer === opp) e -= 0.05;
    // Short-hand threat
    if (oppLen === 1) e -= 0.12;
    if (myLen === 1) e += 0.12;
    if (oppLen === 2) e -= 0.04;
  ...
  function searchMove(state, myIdx, opts) {
    ...
    if (
      ...
      if (opts.exactExploit !== false) {
        ...
        if (!cur && !opts.inBrowser) exactMs = Math.max(exactMs, 320); // EXACT_ONLY
      ...
      var exMulti = null;
      var exHybrid = null;
      var softNRoot = (opts.softSamples != null && opts.softSamples > 0)
        ? opts.softSamples
        : (opts.inBrowser ? 2 : 10); // STACK softSamples default 10
