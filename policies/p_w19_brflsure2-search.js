...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      // W19 p_w19_brflsure2: gold 0518/0499 — short FL force sure 2-control
      // re-get full legals (mergedBR dropped expensive 2s)
      if (!cur && hand.length <= 5 && hand.length >= 3) {
        var oF = oppMinHand(state, myIdx);
        var aF = getLegalPlays(hand, null, state.players[myIdx].passed,
          state.isFirstLead, state.firstLeadCard);
        var sF = [], iF, pF;
        for (iF = 0; iF < aF.length; iF++) {
          pF = aF[iF]; if (!pF) continue;
          if (pF.length === 2 && pF[0].rank === 12 && pF[1].rank === 12) sF.push(pF);
          else if (oF === 2 && pF.length === 1 && pF[0].rank === 12) sF.push(pF);
        }
        if (sF.length) leg = sF;
      }
    } else {
