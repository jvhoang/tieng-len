...
(function (root, factory) {
  ...
  function orderLegals(legals, state, myIdx) {
    // ovkgap singles + combat near-tie companion (no free-lead)
    var hand = state.players[myIdx].hand;
    var cur = state.currentCombo;
    function top(p) {
      if (!p || !p.length) return 99;
      var t = -1;
    ...
    if (cur && cur.type === 'single') {
      var minT = 99, maxT = -1, nSing = 0;
      for (var i = 0; i < legals.length; i++) {
        if (legals[i] && legals[i].length === 1) {
          nSing++;
          var t = top(legals[i]);
          if (t < minT) minT = t;
          if (t > maxT) maxT = t;
      ...
      if (nSing >= 2 && maxT - minT >= 2) forceMin = true;
    }
    return legals.slice().sort(function (a, b) {
      if (!a && b) return 1;
      if (a && !b) return -1;
      if (!a && !b) return 0;
      if (forceMin && a && b && a.length === 1 && b.length === 1) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    // combat-only near-tie min-top (never free-lead)
    if (cur && details.length) {
      ...
      pool.sort(function (da, db) {
        ...
        function top(p){if(!p||!p.length)return 99;var t=-1;for(var i=0;i<p.length;i++)if(p[i].rank>t)t=p[i].rank;return t;}
        var ta = top(a), tb = top(b);
        if (ta !== tb) return ta - tb;
