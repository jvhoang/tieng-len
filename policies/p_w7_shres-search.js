...
(function (root, factory) {
  ...
  function shallowSelfPick(state, myIdx) {
    var hand = state.players[myIdx].hand;
    var cur = state.currentCombo;
    // W7: nest combat deeper (hand>12 only falls to expert)
    if (cur && hand.length > 12) return expertPolicy(state, myIdx);
    var leg = getLegalPlays(hand, cur, state.players[myIdx].passed,
      state.isFirstLead, state.firstLeadCard);
    if (!leg.length) return { pass: true };
    var i;
    ...
    if (!cur) leg = freeLeadCandidates(leg, state, myIdx);
      ...
      var ch = cheapLegals(leg);
      if (ch.length) leg = ch;
      // W7 min-beat: combat singles overkill band — min top before expertScore order
      if (cur.type === 'single' && leg.length >= 2) {
        function top(p){if(!p||!p.length)return 99;var t=-1;for(var j=0;j<p.length;j++)if(p[j].rank>t)t=p[j].rank;return t;}
        var sings = leg.filter(function (p) { return p && p.length === 1; });
        if (sings.length >= 2) {
          var minT = 99, maxT = -1;
          ...
          if (maxT - minT >= 2) {
            leg = sings.slice().sort(function (a, b) {
        ...
        // residual-ish: lower sbc first among cheap multi
    ...
    if (!cur) leg = orderLegals(leg, state, myIdx);
    else if (!(cur.type === 'single')) { /* already ordered */ }
    else if (leg.length && leg[0] && leg[0].length !== 1) leg = orderLegals(leg, state, myIdx);
