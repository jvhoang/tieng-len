...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      leg = mergedBR;
      // W14 p_w14_brflpair6: deep hand>=12 low pairs top<=6
      if (!cur && hand.length >= 12) {
        var lps6 = [];
        for (var i6 = 0; i6 < leg.length; i6++) {
          if (leg[i6] && leg[i6].length === 2 && topRank(leg[i6]) <= 6 &&
              structureBreakCost(hand, leg[i6]) < 8) lps6.push(leg[i6]);
        }
        if (lps6.length) leg = lps6;
      }
    } else {
