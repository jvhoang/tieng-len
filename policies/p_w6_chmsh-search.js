...
(function (root, factory) {
  ...
  function expertPolicy(state, cp) {
    ...
    // W6C CHEAP_MIN_SINGLE: if all cheap are singles with overkill band, hard min-top
    if (cheap.length && cur.type === 'single') {
      var allSing = cheap.every(function (p) { return p && p.length === 1; });
      if (allSing && cheap.length >= 2) {
