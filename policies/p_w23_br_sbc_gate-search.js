...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      if (ch.length) leg = ch;
      // W23 p_w23_br_sbc_gate: gold Series1 residual (0498–0504, 0523–0525)
      // Drop high structure-break answers when any low-break legal remains.
      if (cur && hand.length >= 4 && hand.length <= 12) {
        var lowS = [], highS = [], iS, cS, pS;
        for (iS = 0; iS < leg.length; iS++) {
          pS = leg[iS];
          if (!pS) continue;
          cS = structureBreakCost(hand, pS);
          if (cS < 8) lowS.push(pS);
          else highS.push(pS);
        }
        if (lowS.length >= 1 && highS.length >= 1) leg = lowS;
      }
      leg = orderLegals(leg, state, myIdx);
    }
  ...
  function exploitMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      var ch = cheapLegals(leg);
      if (ch.length) leg = ch;
      // W23 sbc_gate (deep exploit combat path) — same residual filter as root BR
      if (cur && hand.length >= 4 && hand.length <= 12) {
        var lowSd = [], highSd = [], iSd, cSd, pSd;
        for (iSd = 0; iSd < leg.length; iSd++) {
          pSd = leg[iSd];
          if (!pSd) continue;
          cSd = structureBreakCost(hand, pSd);
          if (cSd < 8) lowSd.push(pSd);
          else highSd.push(pSd);
        }
        if (lowSd.length >= 1 && highSd.length >= 1) leg = lowSd;
      }
      leg = orderLegals(leg, state, myIdx);
    }
