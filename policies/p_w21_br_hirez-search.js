...
(function (root, factory) {
  ...
  function residualStruct(hand, play) {
    var used = Object.create(null), i, r, byR = Object.create(null);
    var pairR = 0, maxRun = 0, run = 0;
    if (play) for (i = 0; i < play.length; i++) used[play[i].rank * 4 + play[i].suit] = 1;
    ...
    for (r = 0; r <= 11; r++) {
      if ((byR[r] || 0) >= 2) pairR++;
      if ((byR[r] || 0) >= 1) { run++; if (run > maxRun) maxRun = run; } else run = 0;
    }
    return pairR * 10 + maxRun;
  }
  /**
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      if (ch.length) leg = ch;
      // W21 p_w21_br_hirez: gold 0502/0504 force high residual loose vs low tops
      var ominB = oppMinHand(state, myIdx), ctB = cur.top ? cur.top.rank : 99;
      if (cur && cur.type === 'single' && ctB <= 7 && hand.length >= 7 && ominB >= 4) {
        var bestSc = -1, keepB = [], nB = 0, iB, pB, scB, defB;
        for (iB = 0; iB < leg.length; iB++) {
          pB = leg[iB]; if (!pB || pB.length !== 1) continue;
          nB++; scB = residualStruct(hand, pB);
          if (scB > bestSc) { bestSc = scB; keepB = [pB]; }
          else if (scB === bestSc) keepB.push(pB);
        }
        defB = orderLegals(leg, state, myIdx)[0];
        if (nB >= 2 && keepB.length && defB && defB.length === 1
            && residualStruct(hand, defB) < bestSc) leg = keepB;
      }
      leg = orderLegals(leg, state, myIdx);
    }
