...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      if (ch.length) leg = ch;
      // W22 p_w22_br_2overbreak: gold 0513/0525/0544 — reinject 2 over pure pair-break
      // (W18 br2ctrl never fired: scanned post-cheap where 2s already stripped)
      if (cur && cur.type === 'single' && hand.length >= 5 && hand.length <= 11) {
        var onlyBrk = true, nS = 0, twos = [], i2, p2, c2;
        for (i2 = 0; i2 < leg.length; i2++) {
          p2 = leg[i2]; if (!p2 || p2.length !== 1) continue;
          nS++; c2 = structureBreakCost(hand, p2);
          if (c2 < 12) onlyBrk = false;
        }
        if (onlyBrk && nS >= 1) {
          for (i2 = 0; i2 < fullC.length; i2++) {
            p2 = fullC[i2];
            if (p2 && p2.length === 1 && p2[0].rank === 12) twos.push(p2);
          }
          if (twos.length) leg = twos;
        }
      }
      leg = orderLegals(leg, state, myIdx);
    }
