...
(function (root, factory) {
  ...
  function expertPolicy(state, cp) {
    ...
    // TWO_DEFEND: reclaim with 2 vs mid tops (wider for seat1 combat)
    if (
      cur.type === 'single' &&
      curTop >= 6 &&
      curTop <= 10 &&
      ...
      handLen >= 3 &&
      handLen <= 11 &&
      (infoC.trashCount >= 1 || infoC.control >= 1 || omin <= 2)
    ) {
      twoSingles.sort(function (a, b) { return a[0].suit - b[0].suit; });
      return { play: twoSingles[0] };
    }
    ...
    if (cheap.length) {
      if (cur && cur.type === 'single' && hand.length >= 8 && oppMinHand(state, myIdx) >= 5) {
        ...
        if (proposed && isLegalPlay(proposed) && !playIsExpensive(proposed)) {
          if (topRank(proposed) - cTopG2 > 2 && minTop - cTopG2 <= 2) {
            return orderLegals(cheap.filter(function (p) { return topRank(p) <= minTop; }), state, myIdx)[0];
          }
          return proposed;
        }
      ...
      for (var t = 0; t < nTry; t++) {
        ...
        while (!s.roundOver && steps < 200) {
          ...
          if (cp === myIdx) {
            ...
            if (strongSelf || !curSelf || hSelf <= 8) dec = shallowSelfPick(s, myIdx);
