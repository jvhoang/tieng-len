...
(function (root, factory) {
  ...
  function expertPolicy(state, cp) {
    ...
    if (cheap.length) {
      // structure fold mid multi when all cheap smash
      if (handLen >= 8 && omin >= 4 && curTop < 9 && cur.type !== 'single' && !playIsBomb(cur.cards || [])) {
        var minSbc = 1e9;
        for (var ci = 0; ci < cheap.length; ci++) {
          var sc = structureBreakCost(hand, cheap[ci]);
          if (sc < minSbc) minSbc = sc;
        }
        if (minSbc >= 8) return { pass: true };
      }
  ...
  function pickFreeLeadHard(leg, state, cp) {
    ...
    if (multi.length) {
      ...
      // FORCE_LOW_PAIR: deep hand lead lowest pair if any pair top<=7 (residual volume)
      if (handLen >= 10) {
        var lowP = multi.filter(function (p) { return p.length === 2 && topRank(p) <= 7; });
        if (lowP.length) {
          lowP.sort(function (a, b) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      // EXPERT_FIRST_FL: deep free-lead use hard expert (BR was ignoring low-pair pins)
      if (hand.length >= 9) {
        var hardFl = pickFreeLeadHard(leg, state, myIdx);
        if (hardFl) return { play: hardFl, stats: { mode: 'br-expert-fl' } };
      }
    ...
    for (var ai = 0; ai < actions.length; ai++) {
      ...
      // Combat residual soft-tie STRONG + free-lead pin rate bias
      if (act && cur) {
        multiTie -= Math.min(0.02, structureBreakCost(root.players[myIdx].hand, act) * 0.001);
        if (!playIsExpensive(act)) multiTie += 0.003;
        try {
          var tpx=-1; for (var xj=0;xj<act.length;xj++) if(act[xj].rank>tpx) tpx=act[xj].rank;
          multiTie -= tpx * 0.0003;
        } catch(e){}
      }
      if (act && !cur && act.length === 2 && topRank(act) <= 6) {
        multiTie += 0.02; // strong pin rate for low pairs in BR
      }
      if (act && !cur && act.length === 1) {
        try {
          var ib = analyzeHand(root.players[myIdx].hand);
          if (isTrashSinglePlay(act, ib)) multiTie += 0.015;
        } catch(e2){}
      }
