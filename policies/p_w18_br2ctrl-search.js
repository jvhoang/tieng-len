...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      if (ch.length) leg = ch;
      // W18 p_w18_br2ctrl: gold 0516 — force single-2 when pair-break single exists
      if (cur && cur.type === 'single' && hand.length <= 8) {
        var twos = [], brk = false, i3, p3;
        for (i3 = 0; i3 < leg.length; i3++) {
          p3 = leg[i3]; if (!p3 || p3.length !== 1) continue;
          if (p3[0].rank === 12) twos.push(p3);
          if (structureBreakCost(hand, p3) >= 12) brk = true;
        }
        if (twos.length && brk) leg = twos;
      }
      leg = orderLegals(leg, state, myIdx);
    }
