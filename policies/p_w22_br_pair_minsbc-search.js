...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      if (ch.length) leg = ch;
      // W22 p_w22_br_pair_minsbc: gold 0546 — min-sbc pair only when spread
      if (cur && cur.type === 'pair' && hand.length >= 6
          && oppMinHand(state, myIdx) >= 3) {
        var minC = 1e9, maxC = -1, iP, pP, cP, keep = [];
        for (iP = 0; iP < leg.length; iP++) {
          pP = leg[iP]; if (!pP || pP.length !== 2) continue;
          cP = structureBreakCost(hand, pP);
          if (cP < minC) minC = cP;
          if (cP > maxC) maxC = cP;
        }
        if (maxC > minC) {
          for (iP = 0; iP < leg.length; iP++) {
            pP = leg[iP];
            if (!pP || pP.length !== 2 || structureBreakCost(hand, pP) === minC)
              keep.push(pP);
          }
          if (keep.length) leg = keep;
        }
      }
      leg = orderLegals(leg, state, myIdx);
    }
