...
(function (root, factory) {
  ...
  function expertPolicy(state, cp) {
    ...
    // W24 p_w24_ex_multicontest: gold/holdout — do not soft-pass when same-type multi beat exists
    // Holdout freeze-identical 20300693: PASS on answerable triples while holding multi.
    // Always play best same-type multi when available (midgame, not bomb face).
    if (
      cur &&
      ...
      var multiAns = [], im, pm, cm;
      for (im = 0; im < leg.length; im++) {
        pm = leg[im];
        cm = detectCombo(pm);
        if (cm && cm.type === cur.type && !playHasTwo(pm) && !playIsBomb(pm)) multiAns.push(pm);
      }
      if (multiAns.length) {
        multiAns.sort(function (a, b) {
          var sa = structureBreakCost(hand, a), sb = structureBreakCost(hand, b);
          if (sa !== sb) return sa - sb;
          return topRank(a) - topRank(b);
        });
        return { play: multiAns[0] };
      }
    }
    ...
    // W24: soft-pass only when NO same-type multi answer (multi already forced above)
    if (handLen >= 10 && curTop < 9 && omin >= 7 && cur.type === 'single') return { pass: true }; // TWO soft-pass
    if (handLen >= 9 && curTop < 9 && omin >= 7 && cur.type === 'single') return { pass: true }; // residual v9.1
