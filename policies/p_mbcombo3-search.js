...
(function (root, factory) {
  ...
  function enforcePolicyGuards(state, myIdx, proposed) {
    ...
    var cheap = cheapLegals(leg);
    if (cheap.length) {
      // GUARD_COMBAT_OVERKILL: if proposed climbs >2 above min legal top, force min
      if (proposed && proposed.length && isLegalPlay(proposed) && !playIsExpensive(proposed)) {
        function top(p){var t=-1;for(var i=0;i<p.length;i++)if(p[i].rank>t)t=p[i].rank;return t;}
        var minTop = 99, mi;
        for (mi = 0; mi < cheap.length; mi++) {
          var tt = top(cheap[mi]);
          if (tt < minTop) minTop = tt;
        }
        if (top(proposed) > minTop + 2 && cur.type === 'single') {
          // force lowest top single among cheap
          var mins = cheap.filter(function (p) { return top(p) === minTop; });
          return orderLegals(mins.length ? mins : cheap, state, myIdx)[0];
        }
        return proposed;
      }
