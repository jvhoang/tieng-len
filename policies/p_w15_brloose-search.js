...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      if (ch.length) leg = ch;
      // W15 p_w15_brloose: gold 0498/0523 — drop structure-break singles when loose exists
      // (stack on bropair FL; combat only; BR unique-max surgery)
      if (cur && cur.type === 'single' && hand.length >= 8 && oppMinHand(state, myIdx) >= 4) {
        var looseS = [], iS;
        for (iS = 0; iS < leg.length; iS++) {
          if (leg[iS] && leg[iS].length === 1 && structureBreakCost(hand, leg[iS]) < 8)
            looseS.push(leg[iS]);
        }
        if (looseS.length) {
          leg = leg.filter(function (p) {
            return !p || p.length !== 1 || structureBreakCost(hand, p) < 8;
          });
        }
      }
      leg = orderLegals(leg, state, myIdx);
