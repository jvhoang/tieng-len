...
(function (root, factory) {
  ...
  /** W31 flvol + W33 fix: FREE low naked pair → volume multi (triple/seq/dseq).
  ...
  function flVolPool(hand, multiOrLeg, state, cp) {
    ...
    for (i = 0; i < multiOrLeg.length; i++) {
      ...
      // Seq only when longest available is exactly 4 (CF 20310666); if maxSeq≥5 leave to
      // multi-always/flshort (B reverse 20410397 was 4-seq while 5-seq existed).
      var isVol = false;
  ...
  /** W35 flhidefer: FREE defer high pair for low shed single (gold 0538/0540).
  ...
  function pickPairHi(hand, cur, leg, state, cp) {
    ...
    if (gap > 4) return null; // allow 55→66 style
    // max top must be modest race package (≤9 = J), not AA/KK burn
    if (topRank(best) > 10) return null; // allow up to K pair, not AA
    return best;
  }
  ...
  function pickFreeLeadHard(leg, state, cp) {
    ...
    if (multi.length) {
      ...
      if (flv) return flv;
      // W35 flhidefer: gold 0538/0540 — FREE defer high pair for low shed single
      // CF 20290721@0 s5: QQ → 9D WIN. After flvol/flshort; before multi-always.
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      // W35 flhidefer BR: strip FREE to low shed singles when high pair temptation exists
      if (!cur && !forcedLP) {
        var hideBR = pickFlHidefer(hand, leg, state, myIdx);
        if (hideBR) {
          var lowS = [], iH, pH;
          for (iH = 0; iH < leg.length; iH++) {
            pH = leg[iH];
            if (pH && pH.length === 1 && pH[0].rank <= 6 && !playHasTwo(pH)) lowS.push(pH);
          }
          if (lowS.length) { leg = lowS; forcedLP = false; }
        }
      }
  ...
  function searchMove(state, myIdx, opts) {
    ...
    // W35 flhidefer: FREE high-pair defer before exact-endgame/soft multi
    // (exact-endgame dual-nulled CF 20290721 by returning QQ over trash).
    if (!cur) {
      var hideRoot = pickFlHidefer(hand, legals, state, myIdx);
      if (hideRoot) {
        return { play: hideRoot, stats: { mode: 'flhidefer-hard', via: 'search-root' } };
      }
    }
    ...
    if (opts.bestResponse === false) {
      ...
      if (br && br.play !== undefined) {
        ...
        // W35 flhidefer: hard FREE high-pair defer — do not let soft multi-tie re-pick QQ
        // (CF 20290721: ensemble soft multi beat BR trash and dual-nulled convert).
        if (!cur) {
          var hideHard = pickFlHidefer(hand, legals, state, myIdx);
          if (hideHard) {
            br.play = hideHard;
            br.stats = br.stats || {};
            br.stats.via = 'flhidefer-hard';
