...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      // W6 BR_SMASH_STRIP_PASS: if all cheap smash structure, strip plays + force pass cand
      var smashPass = false;
      if (cur && ch && ch.length) {
    ...
    var allowPass = !!cur && (
      (function () {
        // recompute smash for allowPass when combat path set leg empty
        var ch2 = cheapLegals(getLegalPlays(hand, cur, state.players[myIdx].passed, state.isFirstLead, state.firstLeadCard));
        if (ch2.length === 0) return true;
        var hL2 = hand.length, oM2 = oppMinHand(state, myIdx), cT2 = cur.top ? cur.top.rank : 0;
        if (hL2 >= 9 && oM2 >= 5 && cT2 < 9 && cur.type !== 'single' && !playIsBomb(cur.cards || [])) {
          if (ch2.every(function (p) { return structureBreakCost(hand, p) >= 10; })) return true;
        }
        return false;
      })()
    );
