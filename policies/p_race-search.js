...
(function (root, factory) {
  ...
  function expertPolicy(state, cp) {
    ...
    // p_race micro: if tied/ahead in card race, contest rather than soft-pass mid.
    // Affects expert + BR self rollouts (strongSelf:false under fair dual).
  ...
  function leafEval2p(state, myIdx) {
    ...
    // p_race ONE-AXIS: leafEval race+ (v9.2 race coefficients).
    // Under pre-series-2 maxbudget (perfect+exact+BR-asym) this was dual-neutral;
    // under FAIR dual (hidden, exact off, softSamples>0) leafEval ranks soft/BR tails.
    var e = 0.5 + (oppLen - myLen) * 0.068; // race+ (was 0.056)
    e += (info.twos - oinfo.twos) * 0.065;
    e += (info.control - oinfo.control) * 0.032; // ctrl+
    e += (oinfo.trashCount - info.trashCount) * 0.020; // trash+
    // Lead is valuable
    if (state.currentCombo == null && state.currentPlayer === myIdx) e += 0.05;
    if (state.currentCombo == null && state.currentPlayer === opp) e -= 0.05;
    // Short-hand threat (race endgame)
    if (oppLen === 1) e -= 0.12;
    if (myLen === 1) e += 0.12;
    if (oppLen === 2) e -= 0.04;
