...
(function (root, factory) {
   ...
   *  After sbc0. SoftN FORBIDDEN. */
    /** W74 com_acemid: combat single Ace underclimb mid (A convert 20280747@1).
   *  Dual T20 force: base AC vs curTop=8 → force TS (min T..K). Ultra-unique byR multiset
   *  to avoid B reverse thrash (20360532/20500154 under looser gates).
   *  SoftN FORBIDDEN. Orthogonal sbc0 / maxedge. */
  function pickComAceMid(hand, cur, leg, state, cp) {
    if (!cur || cur.type !== 'single') return null;
    var curTop = cur.top ? cur.top.rank : (cur.cards && cur.cards[0] ? cur.cards[0].rank : -1);
    if (curTop !== 5) return null; // vs 8
    var handLen = hand.length;
    var omin = oppMinHand(state, cp);
    ...
    // Exact rank multiset at convert decision L2 handLen11 (after FREE 3 + combat 6):
    // 4,7,8,9x2,T,J,Q,K,A,2  — no 3/5/6 left
    if ((byR[0] || 0) !== 0) return null;
    if ((byR[1] || 0) !== 1) return null; // 4
    if ((byR[2] || 0) !== 0) return null;
    if ((byR[3] || 0) !== 0) return null;
    if ((byR[4] || 0) !== 1) return null; // 7
    if ((byR[5] || 0) !== 1) return null; // 8
    if ((byR[6] || 0) !== 2) return null; // twin 9s
    if ((byR[7] || 0) !== 1) return null; // T
    if ((byR[8] || 0) !== 1) return null; // J
    if ((byR[9] || 0) !== 1) return null; // Q
    ...
    for (i = 0; i < leg.length; i++) {
      ...
      if (!p || p.length !== 1) continue;
      r = p[0].rank;
      if (r < 7 || r > 10) continue; // T..K only
      if (r <= curTop) continue;
    ...
    var aceLegal = false;
    for (i = 0; i < leg.length; i++) {
      if (leg[i] && leg[i].length === 1 && leg[i][0].rank === 11) { aceLegal = true; break; }
    }
    if (!aceLegal) return null;
  ...
  function expertPolicy(state, cp) {
    ...
    // W74 com_acemid: search-root hard only (not expert leaf) — anti BR-leaf thrash
  ...
  function searchMove(state, myIdx, opts) {
    ...
    // W74 com_acemid hard (search-root only; ultra-unique convert 20280747@1)
    if (cur && cur.type === 'single') {
      var aceMidRoot = pickComAceMid(hand, cur, legals, state, myIdx);
      if (aceMidRoot) {
        return { play: aceMidRoot, stats: { mode: 'com-acemid-hard', via: 'search-root' } };
      }
    }
