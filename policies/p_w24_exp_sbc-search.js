...
(function (root, factory) {
  ...
  /** W24 residual after play — for hard combat residual force sort */
  ...
  function expertPolicy(state, cp) {
    ...
    // W24 p_w24_exp_sbc / EXPERT_COMBAT_RESIDUAL_FORCE (gold Series1 + playlog mass)
    // Hard residual sort on same-type cheap answers — changes freeze-identical leaf.
    // Gates: mid hand, not race, not bomb face; always play (not W16 pass).
    if (cheap.length) {
      if (
        handLen >= 7 && omin >= 4 &&
        curTop < 10 &&
        ...
        for (ip = 0; ip < cheap.length; ip++) {
          pp = cheap[ip];
          cp2 = detectCombo(pp);
          if (cp2 && cp2.type === cur.type) pool.push(pp);
        }
        if (pool.length >= 2) {
    ...
    if (non2.length) {
      ...
      // W24 residual force on non-2 path (same-type mid combat)
      if (
        handLen >= 7 && omin >= 4 &&
        curTop < 10 &&
        !playIsBomb(cur.cards || []) &&
        ...
        var poolN = [], iN, pN, cN;
        for (iN = 0; iN < non2.length; iN++) {
          pN = non2[iN];
          cN = detectCombo(pN);
          if (cN && cN.type === cur.type) poolN.push(pN);
        }
        if (poolN.length >= 2) {
          poolN.sort(function (a, b) {
            var ra = residualAfter(hand, a), rb = residualAfter(hand, b);
            if (ra.sbc !== rb.sbc) return ra.sbc - rb.sbc;
            if (ra.pairR !== rb.pairR) return rb.pairR - ra.pairR;
            if (ra.maxRun !== rb.maxRun) return rb.maxRun - ra.maxRun;
