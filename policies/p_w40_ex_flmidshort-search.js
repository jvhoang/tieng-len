...
(function (root, factory) {
  ...
  /** W40 flmidshort: FREE long mid-seq (len 5–6 after flshort) → short residual 3-seq.
   *  Dual-safe CF B 20360532@1: base 5-seq 6789T → force 3-seq 456 WIN vs v91.
   *  Gold 0545/0548 split residual. Does not touch maxSeq≥7 (flshort owns). */
  function pickFlMidshort(hand, multiOrLeg, state, cp) {
    ...
    var maxSeq = 0, longs = [], shorts = [], i, p, c;
    for (i = 0; i < multiOrLeg.length; i++) {
      p = multiOrLeg[i];
      if (!p || playIsExpensive(p) || playHasTwo(p) || playIsBomb(p)) continue;
      c = detectCombo(p);
      if (!c || c.type !== 'seq') continue;
      if (p.length > maxSeq) maxSeq = p.length;
      if (p.length >= 5 && p.length <= 6) longs.push(p);
      if (p.length === 3 && topRank(p) <= 6) shorts.push(p);
    }
    // Fire when a mid-long seq exists (5–6). If max≥7, flshort already preferred 5;
    // still allow further shorten to 3 when residual trash/pair remains.
    if (!longs.length || !shorts.length) return null;
    if (maxSeq < 5) return null;
    // Prefer min-top short 3-seq (leave high cards)
  ...
  function pickFreeLeadHard(leg, state, cp) {
    ...
    if (multi.length) {
      ...
      if (flh) return flh;
            // W40 flmidshort: long mid-seq → residual 3-seq (after flvol/hidefer)
      var flm = pickFlMidshort(hand, multi, state, cp);
      if (flm) return flm;
// v8.5 free-lead: if 2p and we can see opp hand, prefer unanswerable multi first
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      // Skip if flshort already stripped (leg is short5-only).
      if (!cur && !(shortBR && shortBR.length)) {
        ...
        if (volBR.length) {
          ...
          forcedLP = false;
            ...
            // W40 flmidshort BR: FREE strip to residual 3-seq pool when mid-long seq temptation
      if (!cur && !forcedLP) {
        var midBR = pickFlMidshort(hand, fullMultiBR.length ? fullMultiBR : leg, state, myIdx);
        if (midBR) {
          var short3 = [], iS, pS, cS;
          for (iS = 0; iS < (fullMultiBR.length ? fullMultiBR.length : leg.length); iS++) {
            pS = fullMultiBR.length ? fullMultiBR[iS] : leg[iS];
            if (!pS || playIsExpensive(pS) || playHasTwo(pS)) continue;
            cS = detectCombo(pS);
            if (cS && cS.type === 'seq' && pS.length === 3 && topRank(pS) <= 6) short3.push(pS);
          }
          // also scan full leg for 3-seqs
          for (iS = 0; iS < leg.length; iS++) {
            pS = leg[iS];
            if (!pS || playIsExpensive(pS) || playHasTwo(pS)) continue;
            cS = detectCombo(pS);
            if (cS && cS.type === 'seq' && pS.length === 3 && topRank(pS) <= 6) {
              var sigS = playSig(pS), dup = false, iD;
              for (iD = 0; iD < short3.length; iD++) if (playSig(short3[iD]) === sigS) { dup = true; break; }
              if (!dup) short3.push(pS);
            }
          }
          if (short3.length) { leg = short3; forcedLP = false; }
        }
      }
      ...
      // W30 pairhi: max-top pair force before mulowg min-top
      var phBR = pickPairHi(hand, cur, leg, state, myIdx);
      if (phBR) {
        leg = [phBR];
      } else
