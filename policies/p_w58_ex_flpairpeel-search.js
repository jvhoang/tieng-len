...
(function (root, factory) {
  ...
  /** W58 fl_pairpeel: FREE short-hand double-2 naked low pair → single peel.
   *  CF B 20480208@0 s3: base 6S6C → 6S WIN. fl_twoshed requires handLen≥12/omin≥10.
   *  Gates: FREE, handLen===8, omin===8, twos===2, naked pair top≤3 count===2,
   *  no seq3 top≤5, pick min-suit single of that pair rank. SoftN FORBIDDEN. */
  function pickFlPairPeel(hand, multiOrLeg, state, cp) {
    if (!multiOrLeg || !multiOrLeg.length) return null;
    var handLen = hand.length;
    var omin = oppMinHand(state, cp);
    if (handLen !== 8 || omin !== 8) return null;
    var info = analyzeHand(hand);
    if (info.twos !== 2) return null;
    var byR = {}, i, p, c, top, r;
    for (i = 0; i < hand.length; i++) {
  ...
  function pickFreeLeadHard(leg, state, cp) {
    ...
    if (multi.length) {
      ...
      if (fts) return fts;
      // W58 fl_pairpeel: short-hand double-2 low pair → single (disjoint twoshed hl≥12)
      var fpp = pickFlPairPeel(hand, multi.length ? multi.concat(leg) : leg, state, cp);
      if (fpp) return fpp;
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      var qpcBR = (!sbc0BR && !meBR && !euBR) ? pickComQPairClimb(hand, cur, leg, state, myIdx) : null;
      var shBR = (!sbc0BR && !meBR && !euBR && !qpcBR) ? pickSeqHi(hand, cur, leg, state, myIdx) : null;
      var shrBR = (!sbc0BR && !meBR && !euBR && !qpcBR && !shBR) ? pickSeqHiRes(hand, cur, leg, state, myIdx) : null;
      var saBR = (!sbc0BR && !meBR && !euBR && !qpcBR && !shBR && !shrBR) ? pickSeqAdj(hand, cur, leg, state, myIdx) : null;
      var s5BR = (!sbc0BR && !meBR && !euBR && !qpcBR && !shBR && !shrBR && !saBR) ? pickSeq5Adj(hand, cur, leg, state, myIdx) : null;
      var smuBR = (!sbc0BR && !meBR && !euBR && !qpcBR && !shBR && !shrBR && !saBR && !s5BR) ? pickSeqMidUnder(hand, cur, leg, state, myIdx) : null;
      var phBR = (!sbc0BR && !meBR && !euBR && !qpcBR && !shBR && !shrBR && !saBR && !s5BR && !smuBR) ? pickPairHi(hand, cur, leg, state, myIdx) : null;
      if (sbc0BR) {
        leg = [sbc0BR];
      } else if (meBR) {
        leg = [meBR];
  ...
  function searchMove(state, myIdx, opts) {
    ...
    // W58 fl_pairpeel hard: short-hand double-2 pair peel
    if (!cur) {
      var pairpeelRoot = pickFlPairPeel(hand, legals, state, myIdx);
      if (pairpeelRoot) {
        return { play: pairpeelRoot, stats: { mode: 'fl-pairpeel-hard', via: 'search-root' } };
      }
    }
