    ...
    for (var ai = 0; ai < actions.length; ai++) {
      ...
      for (var t = 0; t < nTry; t++) {
        ...
        while (!s.roundOver && steps < 200) {
          ...
          var dec;
          if (cp === myIdx) {
            dec = strongSelf ? shallowSelfPick(s, myIdx) : expertPolicy(s, cp);
            // W10 brpass: 1-ply residual pass vs high multi contest
            var sc = s.currentCombo;
            if (
              !strongSelf && sc && dec && dec.play && !dec.pass &&
              (sc.type === 'pair' || sc.type === 'triple') &&
              s.players[myIdx].hand.length >= 8 &&
              oppMinHand(s, myIdx) >= 5 &&
              topRank(dec.play) >= 9
            ) {
              var nPlay = applyPlayFast(s, myIdx, dec.play);
              nPlay.isFirstLead = false;
              var nPass = passFast(s, myIdx);
              nPass.isFirstLead = false;
              if (leafEval2p(nPass, myIdx) > leafEval2p(nPlay, myIdx) + 0.05)
                dec = { pass: true };
            }
          } else {
            dec = oppPol;
