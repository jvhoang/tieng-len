...
(function (root, factory) {
  ...
  /** W28 residual after play — residual-max multi sort (gold 0503/0517/0545) */
  ...
  function cmpResidualMulti(hand, a, b, curTop) {
    ...
    // prefer shorter multi when residual ties (leave more packages)
    if (a.length !== b.length) return a.length - b.length;
    var ta = topRank(a), tb = topRank(b);
    if (curTop != null) {
      var ga = ta - curTop, gb = tb - curTop;
  ...
  function expertPolicy(state, cp) {
    ...
    // W28 p_w28_ex_multires: gold residual-max multi among ≥2 same-type multi answers
    // (0503/0517/0545). Fires when multi alternatives exist — unique multi still dual-null.
    if (cur && cur.type !== 'single' && !playIsBomb(cur.cards || [])) {
      var multiPool = [], iM, pM, cM;
      for (iM = 0; iM < leg.length; iM++) {
        pM = leg[iM];
        cM = detectCombo(pM);
        if (cM && cM.type === cur.type && !playHasTwo(pM) && !playIsBomb(pM)) multiPool.push(pM);
      }
      if (multiPool.length >= 2) {
        multiPool.sort(function (a, b) {
          return cmpResidualMulti(hand, a, b, curTop);
        });
        return { play: multiPool[0] };
      }
    }

    var cheap = cheapLegals(leg);
    if (cheap.length) {
      // same residual sort among cheap multi of face type
      if (cur && cur.type !== 'single' && !playIsBomb(cur.cards || [])) {
        var cheapM = [], iC, pC, cC;
        for (iC = 0; iC < cheap.length; iC++) {
          pC = cheap[iC];
          cC = detectCombo(pC);
          if (cC && cC.type === cur.type) cheapM.push(pC);
        }
        if (cheapM.length >= 2) {
          cheapM.sort(function (a, b) {
            return cmpResidualMulti(hand, a, b, curTop);
          });
          return { play: cheapM[0] };
        }
      }
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      if (ch.length) leg = ch;
      // W28 multires: residual-max sort same-type multi cands for BR root
      if (cur && cur.type !== 'single' && !playIsBomb(cur.cards || [])) {
        var brM = [], iB, pB, cB;
        for (iB = 0; iB < leg.length; iB++) {
          pB = leg[iB];
          cB = detectCombo(pB);
          if (cB && cB.type === cur.type && !playHasTwo(pB) && !playIsBomb(pB)) brM.push(pB);
        }
        if (brM.length >= 2) {
          var curTopBR = cur.top ? cur.top.rank : 0;
          brM.sort(function (a, b) {
            return cmpResidualMulti(hand, a, b, curTopBR);
          });
          leg = brM;
        } else {
          leg = orderLegals(leg, state, myIdx);
