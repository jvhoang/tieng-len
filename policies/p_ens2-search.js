...
(function (root, factory) {
  ...
  function structureBreakCost(hand, play) {
    ...
    for (var k = 0; k < keys.length; k++) {
      ...
      // GOLD_SURGICAL inflate: pair smash 14/8 (was 8/5)
    ...
    // GOLD_SURGICAL: run interior 16 for ≥3-chain (else 10); ≥3-chain edge 8 (else 4)
    if (play.length === 1) {
      var pr0 = play[0].rank;
  ...
  function orderLegals(legals, state, myIdx) {
    ...
    return legals.slice().sort(function (a, b) {
      ...
      if (a && !b) return -1;
      if (!a && !b) return 0;
      // Combat: structureBreakCost → min top → expertScore
      if (cur) {
  ...
  function expertPolicy(state, cp) {
    ...
    // p_ens2 multi combat fold: handLen≥9, omin≥5, mid multi top≤9, minSbc≥10 → pass
    if (
      handLen >= 9 &&
      omin >= 5 &&
      curTop <= 9 &&
      ...
      var poolFold = cheapLegals(leg);
      if (!poolFold.length) poolFold = leg;
      var minSbcF = 999;
      for (var fi = 0; fi < poolFold.length; fi++) {
        var scF = structureBreakCost(hand, poolFold[fi]);
        if (scF < minSbcF) minSbcF = scF;
      }
      if (minSbcF >= 10) return { pass: true };
    }
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    for (var ai = 0; ai < actions.length; ai++) {
      ...
      // COMBAT_STRUCT_TIE: combat residual structure soft-tie (NOTE-fair-lever-combat-struct)
      // -0.0005*sbc + residual pair/trip/run bonuses. Magnitude << 1/trials.
