...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      // L2s201 P0-B2: free-lead diversity samples — BR must see non-expert multis/trash
      // that freeLeadCandidates filtered out (author/v60 lines never enter tree otherwise).
      ...
      // L2s211 P0-C2: combat diversity samples — BR must score mid-climb singles/pairs
      // that cheap-only roots never see (v60 firstdiff: K/J/T climbs vs our trash/2s).
      // Mirror free-lead P0-B2: expand branch set, do not residual-pack rateV.
      var seenCb = Object.create(null);
      var sci;
      for (sci = 0; sci < leg.length; sci++) seenCb[playSig(leg[sci])] = 1;
      var extrasCb = [];
      var cheapTopSing = -1;
      for (sci = 0; sci < leg.length; sci++) {
        if (leg[sci].length === 1 && !playHasTwo(leg[sci])) {
          if (leg[sci][0].rank > cheapTopSing) cheapTopSing = leg[sci][0].rank;
        }
      }
      for (sci = 0; sci < fullLegCombat.length; sci++) {
        var pc = fullLegCombat[sci];
        if (seenCb[playSig(pc)]) continue;
        if (playIsBomb(pc) || playHasTwo(pc)) continue;
        var sbcC = structureBreakCost(hand, pc);
        if (sbcC >= 5) continue;
        // Mid singles: above cheapest beat, below pure 2/A spam — control climbs
        if (pc.length === 1 && cur && cur.type === 'single') {
          var rSing = pc[0].rank;
          if (rSing >= 6 && rSing <= 11 && rSing > cheapTopSing) extrasCb.push(pc);
          else if (rSing >= 7 && rSing <= 10 && sbcC < 3) extrasCb.push(pc);
        }
        // Mid pairs that can contest without smashing structure
        if (pc.length === 2 && cur && cur.type === 'pair') {
          var tp = topRank(pc);
          if (tp >= 6 && tp <= 11 && sbcC < 4) extrasCb.push(pc);
        }
        // Short seq answers (3) when current is seq — keep structure path in tree
        if (pc.length === 3 && cur && cur.type === 'seq' && sbcC < 4) extrasCb.push(pc);
      }
      // Prefer lower structure cost, then lower top (less overcommit) among extras
      extrasCb.sort(function (a, b) {
    ...
    // L2s211: combat also gets mixed-opp + progressive deep refine (architecture parity
    // with free-lead 0165/0213 family — combat firstdiff vs v60 is still dominant).
    ...
    if (details.length >= 2 && trials >= 10) {
      ...
      // Conditional MC hybrid only when top-2 nearly tied (free-lead 0157; combat gap tighter)
      var gap01 = Math.abs((details[0].rateV || 0) - (details[1].rateV || 0));
      var gapOk = freeLeadRoot ? gap01 <= 0.06 : gap01 <= 0.05;
      if (gapOk) {
        try {
