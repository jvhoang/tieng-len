...
(function (root, factory) {
  ...
  /** W65 fl_midshort: FREE open seq3 midshort no-2 twin-5/7 (A convert 20500153@0).
   *  Base multi-always seq5 → force 345. Gates: FREE, handLen===13, omin===13, twos===0,
   *  byR[2]===2 (5s), byR[4]===2 (7s), maxL===5, pick expertScore-min among seq3.
   *  SoftN FORBIDDEN. */
  function pickFlMidShort(hand, multiOrLeg, state, cp) {
    ...
    seq3.sort(function (a, b) {
      var ta = topRank(a), tb = topRank(b);
      if (ta !== tb) return ta - tb; // min top among seq3
      return expertScore(a, state, cp) - expertScore(b, state, cp);
    });
    return seq3[0];
  }
  ...
  function pickFreeLeadHard(leg, state, cp) {
    ...
    if (multi.length) {
      ...
      if (fq4) return fq4;
      // W65 fl_midshort: FREE open seq3 midshort (no-2 twin-5/7)
      var fmsh = pickFlMidShort(hand, leg, state, cp);
      if (fms) return fms;
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      var saBR = (!sbc0BR && !meBR && !euBR && !ajuBR && !qpcBR && !shBR && !shrBR) ? pickSeqAdj(hand, cur, leg, state, myIdx) : null;
      var s5BR = (!sbc0BR && !meBR && !euBR && !ajuBR && !qpcBR && !shBR && !shrBR && !saBR) ? pickSeq5Adj(hand, cur, leg, state, myIdx) : null;
      var smuBR = (!sbc0BR && !meBR && !euBR && !ajuBR && !qpcBR && !shBR && !shrBR && !saBR && !s5BR) ? pickSeqMidUnder(hand, cur, leg, state, myIdx) : null;
      var phBR = (!sbc0BR && !meBR && !euBR && !ajuBR && !qpcBR && !shBR && !shrBR && !saBR && !s5BR && !smuBR) ? pickPairHi(hand, cur, leg, state, myIdx) : null;
      if (sbc0BR) {
        leg = [sbc0BR];
      } else if (meBR) {
        leg = [meBR];
