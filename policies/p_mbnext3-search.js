...
(function (root, factory) {
  ...
  function expertPolicy(state, cp) {
    ...
    if (cheap.length) {
      // CHEAP_SBC_ALL_FOLD: if all cheap smash structure, fold mid multi/pair
      if (
        handLen >= 9 &&
        omin >= 5 &&
        curTop < 9 &&
  ...
  function pickFreeLeadHard(leg, state, cp) {
    ...
    if (multi.length) {
      ...
      // FL_LOW_PAIR_KEEP_HIGH: dump low pair; keep AA/KK residual
      if (handLen >= 9 && multi.length) {
        ...
        if (hasHighPairRes && lowPairs.length && multiPick && (topRank(multiPick) >= 9 || multiPick.length >= 4)) {
