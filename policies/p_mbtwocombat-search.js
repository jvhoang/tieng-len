...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    for (var ai = 0; ai < actions.length; ai++) {
      ...
      // Combat residual soft-tie STRONG + min-top
      if (act && cur) {
        multiTie -= Math.min(0.025, structureBreakCost(root.players[myIdx].hand, act) * 0.0012);
        if (!playIsExpensive(act)) multiTie += 0.004;
        try {
          var tp=-1; for (var xi=0;xi<act.length;xi++) if(act[xi].rank>tp) tp=act[xi].rank;
          multiTie -= tp * 0.0002;
        } catch(e){}
      }
