...
function defaultGenome() {
  ...
  return {
    ...
    shortHandB: 5, passHandMin: 4, passOppMin: 2, passMargin: 0.08, winProbEdge: 0.02, gen: 0, id: "v1.0-sh-L2s15"
  };
}
  ...
  if (useSearch) {
    try {
      ...
      const searchOpts = {
        ...
        // Fair duals: best-response both seats (H4)
        ...
        bestResponse: opts.bestResponse !== false && (opts.bestResponse || difficulty === 'grandmaster'),
        alphaBeta: opts.alphaBeta,
        brTrials: opts.brTrials
      };
      // Duals pass timeMs:0 + fixed trials — keep zero time so BR uses trial budget only
      if (opts.timeMs === 0) searchOpts.timeMs = 0;
...
const AI_BUILD = {
  id: "v1.0-sh-L2s15",
  stamped: "2026-07-15T09:07:31.427Z",
  label: "Freeze v1.0-sh-L2s15"
};
