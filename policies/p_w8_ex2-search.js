...
(function (root, factory) {
  ...
  function exactEndgameMove(state, myIdx) {
      ...
      if (cp === myIdx) {
        ...
        if (!leg.length) {
          ...
          if (!st.currentCombo) leg = freeLeadCandidates(leg, st, cp);
            ...
            // W8 ex2: re-include single-2s for exact forced-win
            if (st.currentCombo.type === 'single') {
              var fullX = getLegalPlays(st.players[cp].hand, st.currentCombo,
                st.players[cp].passed, false, null), xi, sx = {};
              for (xi = 0; xi < leg.length; xi++) sx[playSig(leg[xi])] = 1;
              for (xi = 0; xi < fullX.length; xi++) {
                if (fullX[xi].length === 1 && fullX[xi][0].rank === 12 && !sx[playSig(fullX[xi])])
                  leg.push(fullX[xi]);
              }
            }
          }
    ...
    if (!state.currentCombo) rootLeg = freeLeadCandidates(rootLeg, state, myIdx);
      ...
      // W8 ex2 root: re-include single-2s
      if (state.currentCombo && state.currentCombo.type === 'single') {
        var fullR = getLegalPlays(state.players[myIdx].hand, state.currentCombo,
          state.players[myIdx].passed, false, null), ri, sr = {};
        for (ri = 0; ri < rootLeg.length; ri++) sr[playSig(rootLeg[ri])] = 1;
        for (ri = 0; ri < fullR.length; ri++) {
          if (fullR[ri].length === 1 && fullR[ri][0].rank === 12 && !sr[playSig(fullR[ri])])
            rootLeg.push(fullR[ri]);
        }
      }
    }
