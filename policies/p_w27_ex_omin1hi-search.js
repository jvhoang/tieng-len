...
(function (root, factory) {
  ...
  function expertPolicy(state, cp) {
    ...
    // W27 p_w27_ex_omin1hi: gold 0532 — opp 1 card: highest structure-safe single first
    // (do not break pair/run for min-beat when a high loose single wins the race).
    // Always play (not pass). Stacks on seqclimb. SoftN forbidden.
    if (cur.type === 'single' && omin === 1 && handLen >= 2) {
      var hiPool = [], hiLoose = [], iH, pH;
      for (iH = 0; iH < leg.length; iH++) {
        pH = leg[iH];
        if (!pH || pH.length !== 1) continue;
        if (pH[0].rank === 12) continue; // prefer non-2 first if any beat exists
        hiPool.push(pH);
        if (structureBreakCost(hand, pH) < 8) hiLoose.push(pH);
      }
      var poolH = hiLoose.length ? hiLoose : hiPool;
      if (!poolH.length) {
        // only 2 answers: play 2 to finish race
        for (iH = 0; iH < leg.length; iH++) {
          pH = leg[iH];
          if (pH && pH.length === 1 && pH[0].rank === 12) poolH.push(pH);
        }
      }
      if (poolH.length) {
        poolH.sort(function (a, b) {
