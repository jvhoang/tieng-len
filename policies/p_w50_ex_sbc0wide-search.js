...
(function (root, factory) {
  ...
  function pickComSbc0(hand, cur, leg, state, cp) {
    ...
    // W50 sbc0wide: widen for CF 20280748@0 s4 KD→AC (hl11 omin11 curTop4).
    // Keep Ace-only unique minS=0 + gap≥4. Prior reverse kill still via Ace-only.
    if (handLen < 7 || handLen > 12) return null;
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      var shBR = (!sbc0BR && !meBR) ? pickSeqHi(hand, cur, leg, state, myIdx) : null;
      var phBR = (!sbc0BR && !meBR && !shBR) ? pickPairHi(hand, cur, leg, state, myIdx) : null;
      if (sbc0BR) {
        leg = [sbc0BR];
      } else if (meBR) {
        leg = [meBR];
