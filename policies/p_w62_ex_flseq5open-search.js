...
(function (root, factory) {
  ...
  /** W62 fl_seq5open: FREE opening maxL===5 with single-2 + AA (A convert 20360531@0).
   *  Base short 345 → force 34567. Disjoint seqopen (twos≥2 + maxL===6).
   *  Gates: FREE, handLen===13, omin===13, twos===1, byR[A]≥2, maxL===5,
   *  pick expertScore-min among maxL seqs. SoftN FORBIDDEN. */
  function pickFlSeq5Open(hand, multiOrLeg, state, cp) {
    if (!multiOrLeg || !multiOrLeg.length) return null;
    var handLen = hand.length;
    var omin = oppMinHand(state, cp);
    ...
    if (maxL !== 5 || seqs.length < 2) return null;
    var cand = [];
    for (i = 0; i < seqs.length; i++) {
  ...
  function pickFreeLeadHard(leg, state, cp) {
    ...
    if (multi.length) {
      ...
      // W62 fl_seq5open: single-2 + AA maxL===5 open (disjoint seqopen double-2/maxL6)
      var fs5 = pickFlSeq5Open(hand, multi.length ? multi.concat(leg) : leg, state, cp);
      if (fs5) return { play: fs5 };
  ...
  function searchMove(state, myIdx, opts) {
    ...
    // W62 fl_seq5open hard FREE
    if (!cur) {
      var seq5openRoot = pickFlSeq5Open(hand, legals, state, myIdx);
      if (seq5openRoot) {
        return { play: seq5openRoot, stats: { mode: 'fl-seq5open-hard', via: 'search-root' } };
      }
    }
