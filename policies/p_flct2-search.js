  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    for (var ai = 0; ai < actions.length; ai++) {
      ...
      if (act && act.length >= 2 && !cur) {
        ...
        // residual pair bias: prefer FL multi that leaves pair ranks
        try {
          var h0 = root.players[myIdx].hand;
          var used = {};
          for (var ui=0;ui<act.length;ui++) used[act[ui].rank]=(used[act[ui].rank]||0)+1;
          var pairLeft=0;
          var byR={};
          for (var hi=0;hi<h0.length;hi++) byR[h0[hi].rank]=(byR[h0[hi].rank]||0)+1;
          for (var rk in byR) {
            var left = byR[rk] - (used[rk]||0);
            if (left >= 2) pairLeft++;
          }
          multiTie += 0.001 * Math.min(3, pairLeft);
        } catch (e) {}
      }
      ...
      if (act && cur) {
        ...
        // min-top micro among combat answers
        try {
          var top=-1; for (var ti=0;ti<act.length;ti++) if(act[ti].rank>top) top=act[ti].rank;
          multiTie -= top * 0.00005;
        } catch (e2) {}
      }
