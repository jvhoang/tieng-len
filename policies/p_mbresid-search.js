...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      if (ominBr > 2) {
        for (var pi = 0; pi < mergedBR.length; pi++) {
          ...
          else if (pp.length === 2 && !playIsExpensive(pp) && topRank(pp) <= 6) pins.push(pp);
        }
      }
      var seenP = {}, pinned = [];
    ...
    for (var ai = 0; ai < actions.length; ai++) {
      ...
      for (var t = 0; t < nTry; t++) {
        ...
        while (!s.roundOver && steps < 200) {
          ...
          if (cp === myIdx) {
            ...
            if (strongSelf || hSelf <= 11) dec = shallowSelfPick(s, myIdx);
            else dec = expertPolicy(s, cp);
          } else {
            dec = oppPol;
          }
          s = applyDecision(s, cp, dec);
      ...
      // Combat residual soft-tie
      if (act && cur) {
        multiTie -= Math.min(0.01, structureBreakCost(state.players[myIdx].hand, act) * 0.0005);
        if (!playIsExpensive(act)) multiTie += 0.0015;
      }
      // RESID_AFTER: hard residual quality of hand after act (independent of binary rates)
      var residB = 0;
      if (act && Array.isArray(act)) {
        try {
          var infoA = analyzeHand(state.players[myIdx].hand);
          // simulate residual structure via sbc inverse + pair ranks left
          var sbc = structureBreakCost(state.players[myIdx].hand, act);
          residB -= Math.min(0.04, sbc * 0.002);
          var by = {}, ii, rr, pairsLeft = 0, maxRun = 0, run = 0;
          for (ii = 0; ii < state.players[myIdx].hand.length; ii++) {
            by[state.players[myIdx].hand[ii].rank] = (by[state.players[myIdx].hand[ii].rank] || 0) + 1;
          }
          for (ii = 0; ii < act.length; ii++) {
            by[act[ii].rank] = (by[act[ii].rank] || 0) - 1;
          }
          for (rr = 0; rr <= 12; rr++) {
            if ((by[rr] || 0) >= 2) pairsLeft++;
            if ((by[rr] || 0) > 0) { run++; if (run > maxRun) maxRun = run; }
            else run = 0;
          }
          residB += Math.min(0.03, pairsLeft * 0.008);
          residB += Math.min(0.02, Math.max(0, maxRun - 2) * 0.005);
        } catch (eR) {}
      }
      var score = rate + multiTie + residB;
    ...
    // BR_NEAR_TIE_BUCKET_MIN: among one-trial near-ties, hard min top→sbc
    if (details.length) { // combat + free-lead near-tie min
      ...
      if (pool.length) bestPlay = actBySig[pool[0].sig];
    }
    return {
      play: bestPlay,
      stats: {
