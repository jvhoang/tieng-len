  ...
  function pickFreeLeadHard(leg, state, cp) {
    ...
    if (multi.length) {
      ...
      pool = pool.slice().sort(function (a, b) {
        ...
        // deep hand: min top multi first (diverge residual long high multi opens)
        if (handLen >= 10 && ta !== tb) return ta - tb;
        if (la !== lb && Math.abs(ta - tb) <= 1) return la - lb; // short first
