...
(function (root, factory) {
  ...
  function expertPolicy(state, cp) {
    ...
    // W19 p_w19_exresmax: gold 0498/0502 — combat singles force max residual structure
    if (cheap.length && cur && cur.type === 'single' && handLen >= 6) {
      var bestSc = -1, bestP = null, iR, pR, scR, nSing = 0;
      for (iR = 0; iR < cheap.length; iR++) {
        pR = cheap[iR];
        if (!pR || pR.length !== 1) continue;
        nSing++;
        scR = residualStruct(hand, pR);
        if (scR > bestSc || (scR === bestSc && bestP &&
            structureBreakCost(hand, pR) < structureBreakCost(hand, bestP))) {
          bestSc = scR; bestP = pR;
        }
      }
      if (bestP && nSing >= 2) {
        var defP = orderLegals(cheap, state, cp)[0];
        if (!defP || defP.length !== 1 || residualStruct(hand, defP) < bestSc)
          return { play: bestP };
      }
    }
