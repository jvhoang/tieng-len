...
(function (root, factory) {
  ...
  function expertPolicy(state, cp) {
    ...
    // p_g1b: play 2 when ALL cheap smash pair or ≥3-run (series-1 combat)
    if (
      cheap.length &&
      cur.type === 'single' &&
      curTop >= 8 &&
      ...
      for (var hi = 0; hi < hand.length; hi++) {
        byR[hand[hi].rank] = (byR[hand[hi].rank] || 0) + 1;
      }
      function isPairOrRun3Smash(p) {
      ...
      for (var ci = 0; ci < cheap.length; ci++) {
        if (!isPairOrRun3Smash(cheap[ci])) { allSmash = false; break; }
      }
      if (allSmash) {
        twoSingles.sort(function (a, b) { return a[0].suit - b[0].suit; });
        return { play: twoSingles[0] };
