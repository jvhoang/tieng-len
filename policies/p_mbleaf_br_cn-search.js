  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    for (var ai = 0; ai < actions.length; ai++) {
      ...
      for (var t = 0; t < nTry; t++) {
        ...
        if (u >= 0.99) wins++;
        leafSum += leafEval2p(s, myIdx);
      }
      var rate = wins / nTry;
      var leafAvg = leafSum / nTry;
      ...
      // BR_LEAF_BLEND: soft structure/race signal among near-equal binary rates
      var score = rate + multiTie + 0.05 * (leafAvg - 0.5);
    ...
    if (details.length && cur) { // combat-only near-tie + leaf blend
