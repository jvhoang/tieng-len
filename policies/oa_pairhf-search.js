...
(function (root, factory) {
  ...
  function expertPolicy(state, cp) {
    ...
    // OA3 PAIR_WAR_HIGH_FOLD: only when all cheap pair answers are high
    if (
      cur.type === 'pair' &&
      handLen >= 8 &&
      omin >= 5 &&
      ...
      var chP = cheapLegals(leg).filter(function (p) { return p && p.length === 2; });
      if (chP.length && chP.every(function (p) {
        var t = -1; for (var i = 0; i < p.length; i++) if (p[i].rank > t) t = p[i].rank;
        return t >= 9;
      })) {
        return { pass: true };
      }
