...
(function (root, factory) {
  ...
  function structureBreakCost(hand, play) {
    ...
    for (var k = 0; k < keys.length; k++) {
      ...
      if (had >= 2 && left === 1 && used === 1) cost += 16;
      if (had === 2 && used === 1) cost += 10;
      // Breaking a triple
  ...
  function expertScore(play, state, myIdx) {
    ...
    score += afterLen * 1.0;
    score += structureBreakCost(hand, play) * 3.5;
  ...
  function pickFreeLeadHard(leg, state, cp) {
    ...
    if (multi.length) {
      ...
      // Prefer pairs over longer multi when deep (force diverge on residual multi dumps)
      var pairs = pool.filter(function (p) { return p.length === 2; });
      var triples = pool.filter(function (p) { return p.length === 3; });
      if (handLen >= 8 && pairs.length) pool = pairs;
      else if (handLen >= 9 && triples.length) pool = triples;
      pool = pool.slice().sort(function (a, b) {
        var ta = topRank(a), tb = topRank(b);
        if (ta !== tb) return ta - tb; // min top among pairs
