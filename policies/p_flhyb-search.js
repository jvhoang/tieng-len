...
(function (root, factory) {
  ...
  function pickFreeLeadHard(leg, state, cp) {
    ...
    if (multi.length) {
      ...
      // p_flhyb ONE-AXIS: free-lead trash hybrid (not only _exploitFlMode hybrid).
      // When deep hand + high multi top + trash singles + control/2s backup, prefer
      // lowest trash single over multi (human freeLeadDiffer trash).
      if (
        trashPlays.length >= 1 &&
      ...
      // Hybrid trash when exploit dual-self scores hybrid mode (ladder residual path).
      if (
