...
(function (root, factory) {
  ...
  function orderLegals(legals, state, myIdx) {
    ...
    return legals.slice().sort(function (a, b) {
      ...
      if (cur && a && b) {
        ...
        // multi-climb: prefer shorter multi among equal top+sbc
  ...
  function expertPolicy(state, cp) {
    ...
    // PAIR_WAR_HIGH_FOLD: fold mid pair-war when only high pairs answer
    if (
      cur.type === 'pair' &&
      handLen >= 8 &&
      omin >= 5 &&
  ...
  function shallowSelfPick(state, myIdx) {
    ...
    // NEST_SHALLOW_PASS: try fold mid multi/pair when deep (forced-win pass)
    if (cur && cur.type !== 'single' && hand.length >= 8 && oppMinHand(state, myIdx) >= 5) {
      var cTopN = cur.top ? cur.top.rank : 0;
      if (cTopN < 9 && !playIsBomb(cur.cards || [])) {
        var pn = passFast(state, myIdx);
        pn.isFirstLead = false;
        if (exploitPlayoutLeaf(pn, myIdx, 200) >= 0.99) return { pass: true };
      }
    }
    for (i = 0; i < leg.length; i++) {
