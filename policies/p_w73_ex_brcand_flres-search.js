...
(function (root, factory) {
  ...
  function bestResponseMove(state, myIdx, opts) {
    ...
    if (!cur) {
      ...
      // W73 br_cand_pin_flres: FREE residual cand REORDER only (full pool, never hard strip).
      // SoftN FORBIDDEN. Fingerprint-first seat 20390450@1 long multi residual.
      (function () {
        if (hand.length < 9 || hand.length > 13) return;
        var ominBR = oppMinHand(state, myIdx);
        if (ominBR < 4) return;
        var infoBR = analyzeHand(hand);
        if (infoBR.twos > 1) return;
        function resPR(play) {
        ...
        var multi = [], iM, pM, maxL = 0, hasShort = false;
        for (iM = 0; iM < mergedBR.length; iM++) {
          pM = mergedBR[iM];
          if (!pM || pM.length < 2 || playIsExpensive(pM) || playIsBomb(pM)) continue;
          multi.push(pM);
          if (pM.length > maxL) maxL = pM.length;
          if (pM.length === 3 || pM.length === 4) hasShort = true;
        }
        if (multi.length < 2 || maxL < 5 || !hasShort) return;
        var def0 = multi.slice().sort(function (a, b) {
          if (a.length !== b.length) return b.length - a.length;
          return topRank(a) - topRank(b);
        })[0];
        multi.sort(function (a, b) {
          var ra = resPR(a), rb = resPR(b);
          if (ra.pairR !== rb.pairR) return rb.pairR - ra.pairR;
          if (ra.maxRun !== rb.maxRun) return rb.maxRun - ra.maxRun;
          if (a.length !== b.length) return a.length - b.length;
          if (topRank(a) !== topRank(b)) return topRank(a) - topRank(b);
          return expertScore(a, state, myIdx) - expertScore(b, state, myIdx);
        });
        if (playSig(multi[0]) === playSig(def0)) return; // must-disagree
        // full reorder: residual multi first, then remaining mergedBR
        var seen = {}, out = [], i;
        for (i = 0; i < multi.length; i++) {
          var sg = playSig(multi[i]);
          if (!seen[sg]) { seen[sg] = 1; out.push(multi[i]); }
        }
        for (i = 0; i < mergedBR.length; i++) {
          sg = playSig(mergedBR[i]);
          if (!seen[sg]) { seen[sg] = 1; out.push(mergedBR[i]); }
        }
        mergedBR = out;
      })();
      leg = mergedBR;
