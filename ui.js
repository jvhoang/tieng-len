 ...
 * Provides rendering, selection, and high-level start/boot for the game.
 * Delegates game actions (play/pass/newRound) to the controller when available.
 */
...
(function (root, factory) {
  ...
  function createUI(opts = {}) {
    ...
    // Internal selection state (UI concern, not game state)
    let selectedCards = [];
    let vsAI = true;
    let numPlayers = 4;
    let humanSeats = [0];
    ...
    function getState() {
      ...
      // last resort, if caller set window.gameState
    ...
    function renderHand(playerIdx) {
      ...
      (p.hand || []).forEach((c) => {
        const el = createCardEl(c, false, selectableForHand);
    ...
    function playSelected() {
      ...
      if (ctrl && typeof ctrl.playHuman === 'function') {
        ...
        playSound('play');
        if (typeof afterHumanMove === 'function') setTimeout(afterHumanMove, 120);
        return;
      }
    }
    ...
    function doPass() {
      ...
      if (ctrl && typeof ctrl.passHuman === 'function') {
        ...
        if (broadcastChannel && broadcastChannel.postMessage) { try { broadcastChannel.postMessage({type:'sync', seat}); } catch(_) {} }
        playSound('pass');
        if (typeof afterHumanMove === 'function') setTimeout(afterHumanMove, 80);
        return;
      }
    }
