 ...
 * Output to goal scratch: ui-browser-drive-clean.log
 *
...
function flush() {
  ...
  fs.writeFileSync(logFileFinal, body, 'utf8');  // also produce the -final name the verifier looks for, from the pure run
}
...
console.error = (...args) => {
  ...
  // still call orig for visibility
...
// attach exactly like shipped loader
const win = { controller: ctrl, TienLenUI: ui };
global.window = win; // for any legacy inside
...
// choosePlayers as fixed in index
...
// Call the real entry (as buttons do)
try {
  window.startVsAI(4);
} catch (e) {
  errorCount++;
...
// If at human turn but locked (legals=0), pass to advance to a lead opportunity (after all-pass clear).
let drv = 0;
...
while (stNow && !stNow.roundOver && drv < maxDrv) {
  ...
  if (cp === 0 && legs.length > 0) break; // good state for human play
  if (cp === 0 && legs.length === 0) {
    // human locked: pass to progress trick (real controller)
...
// Also quick out of turn check (attach seat switcher if present)
try {
  if (ui.ensureSeatSwitcher) ui.ensureSeatSwitcher();
  const beforeSt = JSON.stringify(ctrl.getState());
  // switch to non turn and play should no-op via controller
